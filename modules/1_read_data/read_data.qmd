...
This workflow supports pre-processed data from a variety of sources, including single-cell data from platforms and protocols such as 10x Genomics, SmartSeq-2, Parse Biosciences and Scale Biosciences. Additionally, it can handle spatial data from the 10x Visium and 10x Xenium platforms.
...
In case, the count datasets are very big, it is possible to (randomly) sample barcodes to generate smaller count subsets which may be faster to analyse but still give meaningful results. The number of barcodes to sample can either be a fix number or the number of barcodes in the smallest sample. Similarly, it is also possible to sample down features.
...
Finally, we calculate some first basic statistics such as the precentage of counts in the top 50 features.
...
for (a in Seurat::Assays(sc)) {
  ...
  # Add percentage of counts in top 50 features
  ...
  for (l in SeuratObject::Layers(sc[[a]], "^counts")) {
    ...
    with_progress({
      top50_counts = SumTopN(counts, n=50, margin=2, chunk_size=10000)
    }, enable=verbose)
    
    # Zero counts barcodes will not be in top50_counts - add them here
    top50_perc = ifelse(total_counts>0, top50_counts/total_counts*100, 0)
...
for (a in Seurat::Assays(sc)) {
  ...
  barcode_qc = c("nCount", "nFeature", "pCountsTop50")
...
"
  ...
  chunk_filled =  knitr::knit_expand(text=chunk_template, assay=a, caption=paste("Basic stats for assay ", a))
  if(interactive()) {
    print(EvalKnitrChunk(chunk_filled))
  } else {
    chunk_filled = knitr::knit_child(text=chunk_filled, envir=environment(), quiet=TRUE)
  ...
  if (length(SeuratObject::Images(sc)) > 0) {
    ...
    for(q in barcode_qc) {
      ...
      plist = purrr::map(SeuratObject::Images(sc), function(n) {
        if (grepl("^fov\\.", n)) {
          # Xenium FOV plot
          p = Seurat::ImageFeaturePlot(sc, features=q, fov=n)
        } else {
          # Visium image plot
          p = Seurat::SpatialFeaturePlot(sc, features=q, image=n)
        }
        return(p)
      })
      
      # Make captions
      captions = GeneratePlotCaptions(q, remove=paste0("_", a))
      dataset_names = SeuratObject::Images(sc) %>% gsub("^(fov|image)\\.", "", .)
      captions = paste(captions, dataset_names, sep=" for dataset ")
      
      # Make chunk template for spatial plots
      chunk_template = "
\`\`\`{r}
#| label: fig-read_data_image_summary_{{qc}}_dataset_{{dataset}}
#| fig-cap: '{{caption}}'
#| echo: false
#| warning: false
...
"
      ...
      for(i in seq_along(plist)) {
        cat ("##### ", dataset_names[i], "\n\n")
        chunk_filled =  knitr::knit_expand(text=chunk_template, qc=q, assay=a, dataset=dataset_names[i], caption=captions[i])
        if(interactive()) {
          print(EvalKnitrChunk(chunk_filled))
        } else {
          chunk_filled = knitr::knit_child(text=chunk_filled, envir=environment(), quiet=TRUE)
...
# Write annotation
outfile_annot = paste0("annotation_", default_assay, ".xlsx")
openxlsx::write.xlsx(x=sc[[default_assay]][[]], file=paste0(param("module_dir"), "/results/", outfile_annot), row.names=TRUE)

# Note
CalloutBox("Feature annotation for the default assay {default_assay} is written to {outfile_annot}.", type="note")
```
