...
if (parse_plate_information) {
  ...
  if (plate_information_update_identity) {
    ...
    layer_orig = SeuratObject::Layers(sc)
    sc[["RNA"]] = SeuratObject::JoinLayers(object=sc[["RNA"]])
    
    # If the original data had just one layer, remove it 
    if (length(layer_orig) == 1) SeuratObject::LayerData(sc, layer=layer_orig) = NULL
...
names_nonzero_cells = names(which(sc$nCount_RNA > 0))
n_zero_cells = length(which(sc$nCount_RNA == 0))
if (n_zero_cells > 0) {
...
"
  ...
  if (length(SeuratObject::Images(sc)) > 0) {
    ...
    for(q in barcode_qc) {
      ...
      plist = FeaturePlotSpatial(sc, features=q, combine=FALSE)
      plist = purrr::map(plist, function(p) {
        p =  p + theme(legend.title=element_blank())
        if (grepl(pattern="nCount", q)) {
            p = p + viridis::scale_fill_viridis(trans="log10") + ggtitle(paste(q, "(log10)"))
        } else {
