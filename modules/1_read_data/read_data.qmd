...
The following datasets are defined in the table:
...
<details class="details_box">
  <summary class="details_box">Pre-processing of 10x data with Cell Ranger</summary>
...
<details class="details_box">
  <summary class="details_box">Pre-processing of SmartSeq-2 data</summary>
...
gt(metrics_tables_by_tech[['{{technology}}']], rowname_col='Dataset')
\`\`\`
"
...
<details class="details_box">
  <summary class="details_box">What is Seurat and which information is contained in a Seurat object?</summary>
...
for (i in 1:nrow(datasets)) {
  ...
  for (j in seq_along(counts_lst[[i]])) {
    ...
    if (any(grepl(pattern="_", x=new_feature_names, fixed=TRUE))) {
      warning(FormatString("New feature names contain '_' after renaming for dataset {path}, assay {assay}. All occurences will be replaced with '-'."))
...
if (!is.null(downsample_barcodes_n) & !is.null(downsample_features_n)) {
  MessageBox("Your data has been downsampled to at most {downsample_barcodes_n} barcodes and at most {downsample_features_n} features.")
} else if (!is.null(downsample_barcodes_n)) {
  MessageBox("Your data has been downsampled to at most {downsample_barcodes_n} barcodes.")
} else if (!is.null(downsample_barcodes_n)) {
  MessageBox("Your data has been downsampled to at most {downsample_features_n} features.")
}
```
...
Finally we calculate some basic statistics such as percent counts in top50 features or median counts.
