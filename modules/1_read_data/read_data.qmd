...
# add condition for the samples
sc$condition = as.factor(gsub("[0-9]", "", sc$orig.ident))
...
#))
sc = ScAddColours(sc, colours=list(
  orig.ident=setNames(sample(zeileis_28, size=length(levels(sc$orig.ident)), replace=TRUE), levels(sc$orig.ident)),
  sample=setNames(sample(zeileis_28, size=length(levels(sc$sample)), replace=TRUE), levels(sc$sample)),
  technology=setNames(sample(zeileis_28, size=length(levels(sc$technology)), replace=TRUE), levels(sc$technology)),
  seurat_clusters=setNames(sample(godsnot_102, size=length(godsnot_102)), 1:length(godsnot_102)),
  Phase=c(G1="#FDD086", S="#9FD7E1", G2M="#FF8DA1"),
  condition=setNames(sample(zeileis_28, size=length(levels(sc$condition)), replace=TRUE), levels(sc$condition))
))
...
for (a in Seurat::Assays(sc)) {
  ...
  for (l in SeuratObject::Layers(sc[[a]], search="^counts\\.")) {
    ...
    with_progress({
      counts_median = CalculateMedians(counts, margin=1, chunk_size=2000, fun=CalculateColumnPerc)
    }, enable=verbose)
