...
params:
  ...
  # Default assay. If null, will be set to the first assay.
  default_assay: null
  ...
  # For 10x Visium HD data: Which bin sizes to import.
  visiumhd_bin_sizes:
    - 8
    - 16
...
We next read the data into a Seurat object.
...
for (i in 1:nrow(datasets)) {
  ...
  # For Visium HD; which bin sizes to use
  visiumhd_bin_sizes = param("visiumhd_bin_sizes")
  ...
  counts_lst[[i]] = ReadCounts(path=path,
                               ...
                               visiumhd_bin_sizes=visiumhd_bin_sizes)
...
Next, we create the Seurat object. This is a summary representation of the object:
...
# Default assay
default_assay = param("default_assay")
if (is.null(default_assay)) default_assay = all_assays[1]
...
# First create a Seurat object per assay to get a list (sc)
sc = purrr::map(all_assays, function(a) {
  ...
  barcode_metadata = purrr::map_dfr(seq_along(assay_counts_lst), function(i) {
  ...
  s =  Seurat::CreateSeuratObject(counts=assay_obj, meta.data=barcode_metadata, assay=a, names.delim=NULL, names.field=NULL)
...
# Collect all barcode metadata levels so that they can be restored after the merge in the final Seurat object
barcode_metadata_levels = purrr::map_dfr(sc, function(s) return(s[[]])) %>%
  purrr::map(levels)

# Now merge all assays into one Seurat object
if (length(sc)>1) {
  sc = merge(sc[[1]], sc[[-1]])
} else {
  sc = sc[[1]]
...
sc$orig.ident = factor(sc$orig.ident, levels=unique(barcode_metadata_levels$orig.ident))
sc$sample = factor(sc$sample, levels=unique(barcode_metadata_levels$sample))
sc$technology = factor(sc$technology, levels=unique(barcode_metadata_levels$technology))

# Replace NA in nCount and nFeature with 0
...
counts_per_cell = sc[[]][,paste0("nCount_", Seurat::Assays(sc))] %>% rowSums()
names_nonzero_cells = which(counts_per_cell>0) %>% names()
names_zero_cells = which(counts_per_cell==0) %>% names()
n_zero_cells = length(names_zero_cells)
if (n_zero_cells > 0) {
  sc = subset(sc, cells=names_nonzero_cells)
  CalloutBox("Removed {n_zero_cells} cells with zero counts.", type="note", quote=FALSE)
}
...
# Remove counts_lst and call garbage collector
rm(counts_lst)
invisible(gc())
```

```{r}
#| label: read_data_seurat_object_print
...
For spatial workflows, we add the image information.
...
for (i in 1:nrow(datasets)) {
  ...
  j = match(barcodes, SeuratObject::Cells(sc))
  ...
  if (technology == "10x_visiumhd") {
    # For Visium there is an image for each bin size
    visiumhd_bin_sizes = param("visiumhd_bin_sizes")
    visiumhd_bin_sizes = sprintf("%03d", visiumhd_bin_sizes)
    image_dir = file.path(path, paste0("square_", visiumhd_bin_sizes, "um"), "spatial")
    default_image_assay = paste0(gsub("\\d+$", "", default_assay), visiumhd_bin_sizes)
    image_key = paste0("slice", ".", orig_ident, visiumhd_bin_sizes)
  } else if (technology == "10x_visium") {
    image_dir = file.path(path, "spatial")
    default_image_assay = default_assay
    image_key = paste0("slice", ".", orig_ident)
  ...
  # For Xenium: which coordinate type to use
  spatial_coordinate_type = param("spatial_coordinate_type")
  
  # Read in image(s)
  image_lst = purrr::map(seq_along(image_dir), function(j) {
    with_progress({
      image = ReadImage(image_dir=image_dir[j],
                        technology=technology,
                        assay=default_image_assay[j],
                        barcodes=barcodes,
  ...
  for (j in seq_along(image_lst)) {
    # Update image key(s)
    SeuratObject::Key(image_lst[[j]]) = paste0(gsub("[^[:alnum:]]", "", image_key[j]), "_")
    
    # Add to Seurat object
    sc[[image_key[j]]] = image_lst[[j]]
    
    # If additional barcode metadata is provided, add as well
    barcode_metadata = attr(image_lst[[j]], "barcode_metadata")
    if (!is.null(barcode_metadata)) {
      sc = Seurat::AddMetaData(sc, barcode_metadata)
    }
  }
  
  # Remove image_lst and call garbage collector
  rm(image_lst)
  invisible(gc())
}
```
...
for (a in Seurat::Assays(sc)) {
  ...
  for (l in SeuratObject::Layers(sc[[a]], "^counts")) {
    ...
    with_progress({
      counts_median = CalculateMedians(counts, margin=1, chunk_size=2000, fun=calculate_perc)
    }, enable=verbose)
...
"
  ...
  if (length(SeuratObject::Images(sc)) > 0) {
    ...
    for(q in barcode_qc) {
      ...
      plist = FeaturePlotSpatial(sc, features=q, assay=a, combine=FALSE) %>% purrr::flatten()
      ...
      # Make captions
      orig_idents = names(plist) %>% gsub("^(fov|image|slice)\\.", "", .)
...
openxlsx::write.xlsx(x=sc[[default_assay]][[]], file=file.path(module_dir, "results", outfile), row.names=TRUE)
