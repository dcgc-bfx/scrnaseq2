  ...
  for (j in seq_along(counts_lst[[i]])) {
    ...
    if (rename_features[j] == 'NO') {
      ...
      if (rename_features[j] == "ENSEMBL") {
        ...
        ensembl_annotation$ensembl_symbol = ifelse(is.na(ensembl_annotation$ensembl_symbol) | nchar(ensembl_annotation$ensembl_symbol)==0, ensembl_annotation$ensembl_id, ensembl_annotation$ensembl_symbol)
          
        # Merge to existing metadata
      ...
      # Make sure that values are not NA or empty. If so, raise error
      if (any(is.na(new_feature_names) | nchar(new_feature_names)==0)) {
        stop(FormatString("New feature names contain NA values or are empty for dataset {path}, assay {assay}. Please make sure that the selected feature metadata column contains valid values."))
      }
    }
...
if (!is.null(downsample_barcodes_n) | downsample_barcodes_equally) {
  ...
  for (i in seq(counts_lst)) {
    for(j in seq_along(counts_lst[[i]])) {
      ...
      counts_lst[[i]][[j]] = counts_lst[[i]][[j]][, keep, drop=FALSE]
      
      # Subset barcode metadata
      keep = rownames(barcode_metadata) %in% barcodes_per_dataset[[i]]
      barcode_metadata = barcode_metadata[keep,, drop=FALSE]
...
if (!is.null(downsample_features_n)) {
  ...
  for (i in seq(counts_lst)) {
    for(j in seq_along(counts_lst[[i]])) {
      ...
      # Subset counts
      counts_lst[[i]][[j]] = counts_lst[[i]][[j]][rows,, drop=FALSE]
      
      # Subset feature metadata
      feature_metadata = feature_metadata[rows,, drop=FALSE]
...
orig_ident_cols = c(sample(zeileis_28, length(zeileis_28)), sample(godsnot_102, length(godsnot_102)))
orig_ident_cols = orig_ident_cols[1:length(orig_ident_levels)]

sample_levels = levels(sc$sample)
sample_cols = c(sample(zeileis_28, length(zeileis_28)), sample(godsnot_102, length(godsnot_102)))
sample_cols = sample_cols[1:length(sample_levels)]

technology_levels =  levels(sc$technology)
technology_cols = c(sample(zeileis_28, length(zeileis_28)), sample(godsnot_102, length(godsnot_102)))
technology_cols = technology_cols[1:length(technology_levels)]

seurat_clusters_cols = c(sample(godsnot_102, length(godsnot_102)), sample(zeileis_28, length(zeileis_28)))
seurat_clusters_levels = 1:length(seurat_clusters_cols)

sc = ScAddColours(sc, colours=list(
  orig.ident=setNames(orig_ident_cols, orig_ident_levels),
  sample=setNames(sample_cols, sample_levels),
  technology=setNames(technology_cols, technology_levels),
  seurat_clusters=setNames(seurat_clusters_cols, seurat_clusters_levels),
...
for (i in 1:nrow(datasets)) {
  ...
  if (technology == "10x_visiumhd") {
    ...
    image_key = paste0("image", ".", orig_ident, visiumhd_bin_sizes)
