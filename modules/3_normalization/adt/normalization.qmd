...
params:
  ...
  module: "normalization_adt"
    
  # Relative path to the module directory (which contains the qmd file)
  module_dir: "modules/3_normalization/adt"
  ...
  # CLR (Seurat), log (just log-transformation), identity (use raw counts)
...
```{r}
#| label: normalization_adt_preparation
...
In this section, we subsequently run a series of Seurat or other functions for each provided sample:
...
The number of raw ADT (antibody-derived tag) counts per barcode is likewise influenced by technical differences in the capture, tagging and sequencing of surface protein–derived oligonucleotides, especially given the low input and variable antibody binding efficiencies. Thus, comparing raw ADT counts between barcodes may reflect sampling and background‐noise effects rather than true differences in protein abundance. After removing low‐quality barcodes, the primary goal of ADT normalization is to correct for these technical sampling and ambient-antibody effects while preserving genuine cell-to-cell variation in surface marker levels.

Count‐depth scaling remains the simplest and most commonly used strategy for ADTs. Here, for each barcode the count for each antibody is divided by a barcode‐specific “size factor” proportional to the total ADT UMI count in that cell. This yields normalized counts whose sums are equal across all barcodes (often scaled to a constant, e.g. 1,000), putting every cell on the same total‐count baseline and attenuating differences that arise merely from sequencing depth or capture efficiency.

Finally, ADT normalization workflows typically log- or CLR-transform the scaled counts for three key reasons. First, log (or centered log-ratio) distances directly correspond to log fold‐changes in protein abundance, highlighting markers with strong relative differences (e.g. an antibody where cell type A has 200 normalized counts vs. cell type B’s 40, rather than a marker with 2,200 vs. 2,000 counts). Second, log/CLR transformation dampens the mean–variance dependence so that highly abundant antibodies don’t dominate downstream statistics. Lastly, it reduces right‐skewness of the count distribution, helping to satisfy the approximate normality assumptions of many dimensionality-reduction and clustering algorithms.
:::

2. Dependent on the normalization of your choice, we either
...
```{r}
#| label: normalization_adt_normalize
#| results: asis
...
if (normalization_method == "log") {
  ...
  CalloutBox(" CLR transformation was applied.", type="note")
...
} else if (normalization_method == "dsb") {
  # Run dsb normalization, better suited for ADT data
  sc = TransformData(sc,
                     log=FALSE,
                     layer=layer_counts,
                     save=layer_data)
...
```{r}
#| label: fig-normalization_adt_rle_plot_raw
...
```{r}
#| label: fig-normalization_adt_rle_plot_normalized
...
```{r}
#| label: normalization_adt_variable_features
#| results: asis
...
```{r}
#| label: normalization_adt_variable_features_plots
#| results: asis
...
\`\`\`{r}
#| label: fig-normalization_adt_variable_features_{{name}}
#| fig-cap: {{caption}}
#| echo: false
#| warning: false
...
```{r}
#| label: normalization_adt_scale_data
#| results: asis
...
```{r}
#| label: normalization_adt_save_software
...
```{r}
#| label: normalization_adt_save_seurat
...
```{r}
#| label: normalization_adt_finish
