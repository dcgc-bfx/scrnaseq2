...
params:
  ...
  num_barcodes_for_sketching: 5000
...
<summary class="details_box">What do we need normalisation for?</summary>
...
| 2. We assign **cell cycle scores** to each cell based on its normalised expression of G2/M and S phase markers. These scores are visualised in a separate section further below. If specified in the above parameter section, cell cycle effects are removed during scaling (step 3). Cell cycle effects removed for this report: `param$cc_remove`; *all* cell cycle effects removed for this report: `param$cc_remove_all`.

<details class="details_box">

<summary class="details_box">How does removal of cell cycle effects affect the data?</summary>
...
| a. Run standard functions to select **variable genes**, and **scale** normalised gene counts. For downstream analysis it is beneficial to focus on genes that exhibit high cell-to-cell variation, that is they are highly expressed in some cells and lowly in others.
...
<summary class="details_box">What is **SCTransform** special about?</summary>
...
Normalisation method used for this report: `param$norm`; with additional sources of variance regressed out: `param$vars_to_regress`.
...
if (normalisation_method == "lognorm") {
  ...
  CalloutBox("We have used the log normalisation of Seurat.", type="note")
  ...
  CalloutBox("We have used the scran normalization.")
  ...
  CalloutBox("We have used the SCTransform normalization.", type="note")
}
...
To better understand the efficiency of the applied normalisation procedures, we plot the relative log expression of genes in at most `n_cells_rle_plot` randomly selected cells per sample before and after normalisation. This type of plot reveals unwanted variation in your data. The concept is taken from @Gandolfo_2018. In brief, we remove variation between genes, leaving only variation between samples. If expression levels of most genes are similar in all cell types, sample heterogeneity is a sign of unwanted variation.
...
```{r}
#| label: normalization_rna_rle_plots
#| fig-cap:
#|  - "Relative log expression plot for raw counts"
#|  - "Relative log expression plot for normalized counts"
...
<details class="details_box">
  <summary class="details_box">How are variable genes selected?</summary>
...
if(normalisation_method != "SCT" & feature_selection_method != "SCT") {
...
CalloutBox("We have used the {feature_selection_method} method to select {num_variable_features} variable features.", type="note")
```
...
<summary class="details_box">What do we need scaling for?</summary>
...
```{r}
#| label: dimensionality_reduction_rna_scale_data
#| results: asis
...
# SCTransform already scaled data
if(normalisation_method != "SCT") {

  # Scale data for variable features
  sc = Seurat::ScaleData(sc, verbose=verbose, vars.to.regress=vars_to_regress)
  
  # When on-disk matrices are in use, then scaling is merely some operations on the combined matrices
  # However, storing this properly is still bugged
  # Therefore, we convert scaled data into an in-memory matrix
  x = SeuratObject::LayerData(sc, layer="scale.data")
  feature_names = rownames(x)
  barcode_names = colnames(x)
  x = as(x, "matrix")
  rownames(x) = feature_names
  colnames(x) = barcode_names
  SeuratObject::LayerData(sc, assay=Seurat::DefaultAssay(sc), layer="scale.data") = x
}

# Print a message
CalloutBox("The following variables were regressed out during scaling of the data: {vars_to_regress*}.", type="note", type="note")
```

## Sketching

Depending on the size of the datasets, it may not possible anymore to run the downstream analyses on the full dataset. In this case, we can use sketching to reduce the size of the dataset. Sketching is a technique that allows to approximate the full dataset with a smaller subset of cells. This is done by randomly selecting a subset of cells and then running the downstream analyses on this subset. The results are then extrapolated to the full dataset. This technique is especially useful for large datasets, where the full dataset cannot be loaded into memory anymore.
...
if (use_sketching) {
  ...
  CalloutBox("We have used sketching.", type="note")
  print(sc)

  # Finally reset default assay to non-sketched assay
  SeuratObject::DefaultAssay(sc) = ifelse(normalisation_method=="SCT", 
                                          paste(default_assay, "SCT", sep="_"), 
                                          default_assay)
} else {
  CalloutBox("We have not used sketching.", type="note")
}
```
