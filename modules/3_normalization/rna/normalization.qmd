...
if(normalization_method != "SCT" & !use_sketching) {
  ...
  # Unfortunately ScaleData has limited support for on-disk counts: regress out does not work (values are the same with and without)
  # Therefore we load the normalized data into memory, run ScaleData and 
  # then extract the scale.data layer.
  if (on_disk_counts & length(vars_to_regress) > 0) {
...
if (use_sketching) {
  ...
  SeuratObject::Key(sc[[sketched_assay_name]]) = sketched_assay_name %>% 
