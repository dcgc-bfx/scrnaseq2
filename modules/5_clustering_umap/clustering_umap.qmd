...
params:
  ...
  # Which dimensionality reduction to use. If null, will be set to the default reduction of the default assay of the input Seurat object.
  dimensionality_reduction_method: "pca"

  # Number of dimensions to use. 
  dim_n: 10
  
  # Cluster resolutions to use. The first one will be used as default clustering.
  cluster_resolution: 
  - 0.5
  - 0.3
  - 0.7
...
<details class="details_box">
  <summary class="details_box">Further explanation on clustering</summary>
...
cluster_resolutions = param("cluster_resolution") %>% unique()
default_cluster_resolution = cluster_resolutions[1]
dim_n = param("dim_n")

# Construct phylogenetic tree based on an average calculated for each sample
if (sc$orig.ident %>% levels() %>% length() > 1) {
  sc = Seurat::BuildClusterTree(sc, reduction=DefaultReduct(sc), dims=1:dim_n, verbose=verbose)
  Seurat::Misc(sc, slot="trees") = list(orig.ident=Seurat::Tool(sc, "Seurat::BuildClusterTree"))
}

# Find neighbors for clustering
sc = Seurat::FindNeighbors(sc, reduction=DefaultReduct(sc), dims=1:dim_n, verbose=verbose, k.param=param("cluster_k"))

# Find clusters based on the neighbors
sc = Seurat::FindClusters(sc, resolution=cluster_resolutions, algorithm=4, verbose=verbose, method="igraph")

# Construct phylogenetic tree based on an average calculated for each cluster
# Also add colour lists for each clustering
for (r in cluster_resolutions) {
  r = cluster_resolutions[1]
  ...
  if (sc[[n, drop=TRUE]] %>% levels() %>% length() > 1) {
    Seurat::Idents(sc) = n
    sc = Seurat::BuildClusterTree(sc, reduction=DefaultReduct(sc), dims=1:dim_n, verbose=verbose)
    lst = list(Seurat::Tool(sc, "Seurat::BuildClusterTree"))
    names(lst) = n
    suppressWarnings({Seurat::Misc(sc, "trees") = c(Seurat::Misc(sc, "trees"), lst)})
  }

  # Add colour lists for each clustering resolution
  nms = levels(sc[[n, drop=TRUE]])
  col = ScColours(sc, "seurat_clusters")[1:length(nms)]
  names(col) = nms
                                         
  lst = list(col)
  names(lst) = n
  sc = ScAddColours(sc, colours=lst)
}

# Set default clustering
n = paste0(DefaultAssay(sc), "_snn_res.", default_cluster_resolution)
...
# Generate UMAP
sc = Seurat::RunUMAP(sc, reduction=DefaultReduct(sc), dims=1:dim_n, verbose=verbose, umap.method="uwot", n.neighbors=param("umap_k"))
```
