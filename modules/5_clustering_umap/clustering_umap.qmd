...
for(ident in names(plist)) {
    ...
    # Generate captions
    captions = purrr::map(dimensionality_reduction_methods, function(method) return(FormatString("Spatial plot for {ident} coloured by clustering with resolution {res} based on reduction {toupper(method)}"))) %>% purrr::flatten_chr()
