...
for (method in dimensionality_reduction_methods) {
  for (res in sort(unlist(cluster_resolutions))) {
    ...
    chunk_filled =  knitr::knit_expand(text=chunk_template, name=clustering_name, caption=FormatString("UMAP coloured by clustering with resolution {res} based on reduction {toupper(method)}"))
    if (interactive()) {
