...
#| eval: !expr length(dimensionality_reduction_methods) * length(cluster_resolutions) > 0
#| include: !expr length(dimensionality_reduction_methods) * length(cluster_resolutions) > 0
#| results: asis
#| echo: false
...
cat("::: {layout-ncol=", min(length(plist), 3), "}\n", sep="")
...
plist = purrr::map(SeuratObject::Images(sc), function(i) {
    pl = DimPlotSpatial(sc, images=i, group.by=clusterings_to_plot, combine=FALSE, cols=ScColours(sc, clusterings_to_plot))
    pl = purrr::map(seq(pl), function(i) {
        p = pl[[i]] + theme(legend.title=element_blank()) + ggtitle(clusterings_to_plot[i])
        return(p)
    })
...
for(ident in names(plist)) {
    ...
    # If more than one plot, choose two column layout
    if (length(plist[[ident]]) > 1) cat("::: {layout-ncol=2}\n")
    
    # Generate captions
    captions = purrr::map(dimensionality_reduction_methods, function(method) return(FormatString("Spatial plot for dataset {ident} coloured by clustering with resolution {res} based on reduction {toupper(method)}"))) %>% purrr::flatten_chr()
