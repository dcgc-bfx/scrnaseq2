...
# Create a copy named "umap" for the first umap. This allows to run FeaturePlot without specifying the UMAP (makes it easier). 
...
if (use_sketching) {
  ...
  first_umap = NULL
  for (reduct in dimensionality_reductions) {
    reduct = dimensionality_reductions[1]
    
    # Name of sketched and full reduction
    sketched_reduct = gsub("[^_]+_", "", reduct)
    sketched_reduct = paste0(sketched_assay, "_", sketched_reduct) %>% tolower()
    full_reduct = reduct
    
    # Names of the sketched and full clusterings
...
if (length(Seurat::Images(sc)) > 0) {
  ...
  names(plist) = gsub(pattern="^(fov|image)\\.", replacement="", x=image_names)
...
"
  ...
  for(ident in names(plist)) {
      ...
      # Generate captions
      captions = purrr::map(dimensionality_reductions, function(reduct) return(FormatString("Spatial plot for dataset {ident}, dimensionality reduction {reduct}, clustering resolution {cluster_resolutions}"))) %>% purrr::flatten_chr()
...
plist = purrr::map(dimensionality_reductions, function(reduct) {
  ...
  clustering_names = paste0(clustering_prefix, sort(as.numeric(cluster_resolutions)))
...
CalloutBox("Default clustering is set to the dimensionality reduction {dimensionality_reductions[1]} with resolution {cluster_resolutions[1]}.", type="note")
```
