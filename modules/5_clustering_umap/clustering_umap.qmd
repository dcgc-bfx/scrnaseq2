...
plist[["by_cluster"]][["all_samples"]] = Seurat::DimPlot(sc,
                                                         cols=ScColours(sc, "seurat_clusters"),
                                                         label=TRUE,
                                                         label.box=TRUE,
                                                         label.color="white",
...
for (i in names(plist)) {
  for (j in names(plist[[i]])) {
    ...
    # Label clusters if we have coloured by sample (plots coloroured by cluster are already labeled)
    if (i == "by_sample") {
      plist[[i]][[j]]$data$seurat_clusters = barcode_metadata[rownames(plist[[i]][[j]]$data), "seurat_clusters"]
      plist[[i]][[j]] = LabelClusters(plist[[i]][[j]], id="seurat_clusters", box=FALSE)
    }
    
    # Update labels. Increase the number of rows in legend to use the full width of the plot
    if (i == "by_cluster") { 
      plist[[i]][[j]] = plist[[i]][[j]] + 
