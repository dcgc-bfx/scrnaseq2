...
CalloutBox("The resolution parameter {{cluster_resolution}} was used.", type="note")

# Set up colors for default clustering
sc = ScAddColours(sc, colours=list(seurat_clusters=ScColours(sc, default_name)))
```
...
# Set default UMAP
sc[["umap"]] = sc[[paste0(dimensionality_reduction_methods[1], "_umap")]]
SeuratObject::Key(sc[["umap"]]) = "umap_"
...
for (method in dimensionality_reduction_methods) {
  for (res in cluster_resolutions) {
    ...
    chunk_filled =  knitr::knit_expand(text=chunk_template, name=name, caption=paste("Clustering based on", toupper(method), ", with resolution", res))
    if(interactive()) {
