...
# Test different dimensionality reduction methods and clustering resolution parameters
clusterings_to_plot = purrr::map(dimensionality_reduction_methods, function(m) {
    return(paste0(DefaultAssay(sc), "_", m, "_", "snn", cluster_resolutions, ifelse(use_sketching, "_full", "")))
}) %>% purrr::flatten_chr()

plist = purrr::map(SeuratObject::Images(sc), function(i) {
    pl = DimPlotSpatial(sc, images=i, group.by=clusterings_to_plot)
    return(pl)
})
names(plist) = SeuratObject::Images(sc)
