...
Clustering trees help in understanding how clusters split or merge as the resolution changes in the single-cell RNA-sequencing analysis. Each node represents a cluster at a given resolution, and edges between nodes indicate how clusters from one resolution relate to clusters at another resolution. This makes it easier to track cluster stability or transitions, aiding in the selection of an appropriate resolution for downstream analysis.

::: {.callout-tip title="What is the best resolution for my experiment?" collapse="true"}
While making a choice for the best resolution for your experiment, it is important to check at what point the cluster tree is getting messier; which means that there are nodes with multiple incoming edges.
:::
...
clust_tree_plot = clustree(sc, prefix = "RNA_pca_snn") +
  theme(legend.box = "vertical") +
  guides(color = guide_legend(title = "Resolution", position = "bottom"),
         size = "none")

clust_tree_plot + ylim(layer_scales(clust_tree_plot)$y$range$range[1] - 0.2, 
