...
for (assay in Assays(sc)) {

  dimensionality_reductions = param("dimensionality_reductions")
  if (is.null(dimensionality_reductions)) dimensionality_reductions = DefaultReduct(sc, assay=assay)
  cluster_resolutions = param("cluster_resolution") %>% unlist() %>% as.numeric()
  
  for (reduct in dimensionality_reductions) {
    dim_n = min(param("dim_n"), ncol(sc@reductions[[reduct]]@cell.embeddings)) %>% as.integer()
...
cat(":::\n")
```
### Multi modalities clustering resolution

When running the analysis for multi-modalities, it is important to check if all the cells present in all assays are same. This deviation can occure because of individual filtration steps performed previously.
If the same cells are present after filtration, the data can be used as it is else a new dimensionality reduction is performed for all assays.

```{r}
#| label: clustering_umap_multimodal
#| results: asis

if(length(Assays(sc)) > 1) {
  
  assay_names <- Seurat::Assays(sc)
  
  redo_reduct = ifelse(nrow(Embeddings(sc[["rna_pca"]])) == 
                         nrow(Embeddings(sc[["adt_pca"]])), 
                       TRUE, FALSE)

  # Find cells present in all assays
  common_cells <- Reduce(
    f = intersect,
    x = lapply(assay_names, function(assay) colnames(sc[[assay]]))
  )
  
  # Subset the Seurat object
  sc <- subset(sc, cells = common_cells)

  if(!redo_reduct) {
    # to perform reduction again, to do later
  }

  # Graph names (e.g. rna_pca_snn)
  graph_name = paste(Assays(sc), collapse=".")
  graph_name = paste0(graph_name, "_", "pca")
  wknn_graph_name = paste0(graph_name, "_wknn") %>% tolower()
  wsnn_graph_name = paste0(graph_name, "_wsnn") %>% tolower()
  umap_name = paste(paste(Assays(sc), collapse="."), "pca_umap", sep = "_") %>% 
    tolower()
  
  rna_dims <- min(20, ncol(Embeddings(sc[["rna_pca"]])))
  adt_dims <- min(15, ncol(Embeddings(sc[["adt_pca"]])))

  sc = FindMultiModalNeighbors(sc, reduction.list = list("rna_pca", "adt_pca"), 
                               dims.list = list(1:min(rna_dims, dim_n), 
                                                1:min(adt_dims, dim_n)), 
                               knn.graph.name = wknn_graph_name, 
                               snn.graph.name = wsnn_graph_name)

  sc = RunUMAP(sc, nn.name = "weighted.nn", reduction.name = umap_name, 
               reduction.key = "wnnUMAP_")
  
  sc = FindClusters(sc, graph.name = wsnn_graph_name, algorithm = 4, 
                    verbose = FALSE, resolution=sort(cluster_resolutions))
  
  # to make the names consistent with previous formatting
  colnames(sc@meta.data) = gsub("_res.", "", colnames(sc@meta.data))
  
  for(res in cluster_resolutions) {
    if(0 %in% as.character(sc[[paste0(wsnn_graph_name, res)]] %>%
                           pull(paste0(wsnn_graph_name, res)))) {
     sc[[paste0(wsnn_graph_name, res)]] <-
       as.factor(as.numeric(as.character(sc[[paste0(wsnn_graph_name, res)]] %>%
                                           pull(paste0(wsnn_graph_name, res)))) +
                   1)
    }
  }
...
```{r}
#| label: clustering_plots_umap_multimodal
#| results: asis
...
if(length(Assays(sc)) > 1) {
  ...
  captions = c()
  for (res in sort(cluster_resolutions)) {
    clustering_name = paste0("rna.adt_pca_wsnn", res)
    
    # Colours
    bcs = SeuratObject::Cells(sc[[default_assay]])
    barcode_metadata = sc[[]][bcs, ]
    l = levels(barcode_metadata[, clustering_name])
    col = ScColours(sc, "seurat_clusters")[1:length(l)]
    names(col) = l
    col = list(col)
    names(col) = clustering_name
    sc = ScAddColours(sc, colours=col)
      
    plist_multi[[clustering_name]] = Seurat::DimPlot(sc, reduction="rna.adt_pca_umap", group.by=clustering_name, label=TRUE) +
      scale_color_manual(values=col[[clustering_name]]) +
      AddPlotStyle(legend_position="none", title=res)
  }
...
cat("::: {layout-ncol=3}\n", sep="")
  ...
  for (i in seq_along(plist_multi)) {
    chunk_filled =  knitr::knit_expand(text=chunk_template, name=names(plist_multi)[i], caption=captions[i])
    if (interactive()) {
      print(EvalKnitrChunk(chunk_filled))
    } else {
      chunk_filled = knitr::knit_child(text=chunk_filled, envir=environment(), quiet=TRUE)
...
plist = purrr::map(dimensionality_reductions, function(reduct) {
  ...
  assay = ifelse(length(Assays(sc)) > 1, paste0(Assays(sc), collapse="."), default_assay) %>% tolower()
  reduct_short = gsub("[^_]+_", "", reduct)
  clustering_prefix = paste0(assay, "_", reduct_short, "_", ifelse(length(Assays(sc)) > 1, "wsnn", "snn"))
...
default_clustering = paste0(clustering_prefix, res)

# update the seurat clusters and get the colors
sc$seurat_clusters = sc[[default_clustering]]

l = levels(sc$seurat_clusters)
col = ScColours(sc, "seurat_clusters")[1:length(l)]
names(col) = l
col = list(col)
names(col) = default_clustering
sc = ScAddColours(sc, colours=col)

# update the Idents to seurat_clusters
SeuratObject::Idents(sc) = "seurat_clusters"

# Write message about default clustering
CalloutBox("Default clustering for assay {default_assay} is set to the dimensionality reduction {reduct_short} with resolution {res}.", type="note")
CalloutBox("In case of multi modalities, requested combination of assays is used {assay} but default assay is set to {default_assay} with dimensionality reduction {reduct_short} with resolution {res}.", type="note")
...
condition = barcode_metadata[bcs, "condition"]

sample_cells = table(orig_idents)
sample_labels = paste0(levels(orig_idents)," (", sample_cells[levels(orig_idents)],")")
condition_labels = paste0(levels(condition)," (", sample_cells[levels(condition)],")")

# Initialize Plots 
plist=list()
plist[["by_cluster"]] = list()
plist[["by_cluster"]][["all_samples"]] = Seurat::DimPlot(sc, reduction = "umap",
                                                         group.by="seurat_clusters")
plist[["by_sample"]] = list()
plist[["by_sample"]][["all_samples"]] = Seurat::DimPlot(sc, reduction = "umap",
                                                        group.by="orig.ident")

plist[["by_condition"]] = list()
plist[["by_condition"]][["all_samples"]] = Seurat::DimPlot(sc, reduction = "umap",
                                                        group.by="condition")
...
for (i in names(plist)) {
  for (j in names(plist[[i]])) {
    ...
    if (i == "by_condition") { 
      plist[[i]][[j]] = plist[[i]][[j]] + scale_color_manual(values=ScColours(sc, "condition"), labels=sample_labels) +
          guides(col=guide_legend(ncol=7, override.aes=list(size=3)))
    }
  }
}
