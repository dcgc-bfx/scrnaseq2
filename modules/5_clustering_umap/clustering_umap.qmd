...
if (is.null(dimensionality_reductions)) dimensionality_reductions = DefaultReduct(sc)
cluster_resolutions = param("cluster_resolution") %>% unlist()
dim_n = param("dim_n")

# Calculate per dimensionality reduction: PCA, CCA integration, ...
for (method in dimensionality_reductions) {
  nn_graph_name = paste(method, "nn", sep="_") %>% tolower()
  snn_graph_name = paste(method, "snn", sep="_") %>% tolower()
  ...
  for (res in cluster_resolutions) {
    # Rename
    old_name = paste0(method, "_snn_res.", res)
    new_name = paste0(method, "_snn", res)
...
default_name = paste0(dimensionality_reductions[1], "_snn", cluster_resolutions[1])
...
for (method in dimensionality_reductions) {
  ...
  sc = Seurat::RunUMAP(sc, 
                       ...
                       reduction.key=paste0(stringr::str_to_title(method), "_umap"),
                       return.model=use_sketching)
}

# Set default UMAP
default_name = paste0(dimensionality_reductions[1], "_umap")
DefaultVisualization(sc) = default_name
```
...
if (use_sketching) {
  for (method in dimensionality_reductions) {
    sketch_clustering = paste0(method, "_", "snn", cluster_resolutions)
    full_clustering = paste0(method, "_", "snn", cluster_resolutions, "_full")
  ...
  # Set default clustering
  default_name = paste0(dimensionality_reductions[1], "_", "snn", cluster_resolutions[1], "_full")
  sc$seurat_clusters = sc[[]][, default_name]
  SeuratObject::Idents(sc) = "seurat_clusters"
  
  # Set default UMAP
  um = sc[[paste0(dimensionality_reductions[1], "_umap_full")]]
...
for (method in dimensionality_reductions) {
  for (res in cluster_resolutions) {
    ...
    if (use_sketching) {
      n = paste0(method, "_", "snn", res, "_full")
      v = paste0(method, "_umap_full")
    } else {
      n = paste0(method, "_", "snn", res)
...
for (method in dimensionality_reductions) {
  for (res in sort(unlist(cluster_resolutions))) {
    if (use_sketching) {
      name = paste0(method, "_", "snn", res, "_full")
    } else {
      name = paste0(method, "_", "snn", res)
    }
    chunk_filled =  knitr::knit_expand(text=chunk_template, name=name, caption=FormatString("UMAP coloured by clustering with resolution {res} based on reduction {method}"))
    if (interactive()) {
...
CalloutBox("Default clustering is set to the dimensionality reduction {dimensionality_reductions[1]} with the resolution parameter {cluster_resolutions[1]}.", type="note")
```
...
# Test different dimensionality reduction methods and clustering resolution parameters
clusterings_to_plot = purrr::map(dimensionality_reductions, function(method) {
    return(paste0(method, "_", "snn", cluster_resolutions, ifelse(use_sketching, "_full", "")))
}) %>% purrr::flatten_chr()
...
names(plist) = gsub(pattern="^(fov|image|slice)\\.", replacement="", x=image_names)
...
for(ident in names(plist)) {
    ...
    # Generate captions
    captions = purrr::map(dimensionality_reductions, function(method) return(FormatString("Spatial plot for dataset {ident} coloured by clustering with resolution {cluster_resolutions} based on reduction {toupper(method)}"))) %>% purrr::flatten_chr()
...
outfile = paste0("visualization_", default_vis , ".xlsx")
