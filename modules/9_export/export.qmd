...
## Cell metadata and projections

Cell metadata (sample, cluster, ...) and projections (PCA, UMAP, ...)  are exported as `CSV` files. These files can then be imported into other tools. Export is done per dataset (with the original cell IDs) and per aggregated analysis (`all`, where the cell IDs were made unique).

**Notes**: 

- Loupe Browser and Xenium Explorer support the import of these files.
- When using the aggregated analysis (`all`), make sure that the order of samples in the aggregated dataset matches the order of samples in this analysis.
...
- Spatial information is not included in the `cloupe` file. For 10x Visium/VisiumHD data, we recommend to use the original `cloupe` file provided by Spaceranger and to import the `CSV` cell metadata (see above for details) in Loupe.

For 10x Xenium, the `cloupe` file can be loaded into the Loupe Cell Browser to inspect the cell clustering. For spatial information, use the Xenium explorer (see below for details).
...
if (export_cellxgene) {
  ...
  # sceasy::convertFormat does not work anymore so we run its code here directly
  main_layer = "data"
  X = Seurat::GetAssayData(object=adata, assay=default_assay, layer=main_layer)
  obs = sceasy:::.regularise_df(adata@meta.data, drop_single_values=FALSE)
  var = sceasy:::.regularise_df(Seurat::GetAssay(adata, assay=default_assay)@meta.features, drop_single_values=TRUE)
  reductions = names(adata@reductions) 
  obsm = purrr::map(reductions, function(name) as.matrix(Seurat::Embeddings(adata, reduction=name)))
  names(obsm) = paste0("X_", tolower(reductions))
  anndata = reticulate::import("anndata", convert = FALSE)
  adata = anndata$AnnData(X = Matrix::t(X), obs=obs, var=var, obsm=obsm, layers=NULL)
  adata$uns = reticulate::dict(colour_lists)
  adata$write(file.path(module_dir, "results", "export_cellxgene", "cellxgene.h5ad"))
...
To facilitate further analysis, the Seurat object is exported. When counts are stored on disk, they will be exported as well as separate directories that need to be in the same directory as the Seurat object. Alternatively, they can also be included in the Seurat object.
