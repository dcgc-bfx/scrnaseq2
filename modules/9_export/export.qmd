...
Cell metadata (sample, cluster, ...) and projections (PCA, UMAP, ...)  are exported as CSV file. These file can then be imported into other tools. Export is done per dataset (with the original cell IDs) and per aggregated analysis (`all`, where the cell IDs were made unique).
