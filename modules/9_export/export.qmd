...
if (export_loupe & all(grepl("10x", sc$technology))) {
  ...
  categories_to_export = purrr::map(sc[[]], class) %>% unlist()
  categories_to_export = categories_to_export[categories_to_export %in% c("factor", "character")] %>% names()
  ...
  if (all(sc$technology == "10x")) {
    ...
    embeddings = purrr::map(SeuratObject::Reductions(sc), function(r) {
      return(sc[[r]]@cell.embeddings[,1:2])
    })
  ...
  for(smp in levels(sc$orig.ident)) {
    ...
    readr::write_csv(x=categorial_data, 
              file=file.path(module_dir, "results", "loupe", "samples", smp, "Loupe_metadata.csv"))
    
    # Embeddings
    for( r in SeuratObject::Reductions(sc)) {
      emb = sc[[r]]@cell.embeddings[barcodes, 1:2] %>%
        as.data.frame() %>%
        tibble::rownames_to_column(var="barcode")
      emb$barcode = categorial_data$barcode
      readr::write_csv(x=emb,
                file=file.path(module_dir, "results", "loupe", "samples", smp, paste0("Loupe_projection_", r, ".csv")))
    }
  }
  ...
  colnames(categorial_data) = col_nms
  readr::write_csv(x=categorial_data, 
            file=file.path(module_dir, "results", "loupe", "samples", "all", "Loupe_metadata.csv"))
  
  # Embeddings
  for( r in SeuratObject::Reductions(sc)) {
    emb = sc[[r]]@cell.embeddings[, 1:2] %>%
      as.data.frame() %>%
      tibble::rownames_to_column(var="barcode")
    emb$barcode = categorial_data$barcode
    readr::write_csv(x=emb,
              file=file.path(module_dir, "results", "loupe", "samples", "all", paste0("Loupe_projection_", r, ".csv")))
  }
...
if (export_cellxgene) {
  ...
  colour_lists = purrr::map(colour_lists, function(l) {
    # Port color names to hex color codes
