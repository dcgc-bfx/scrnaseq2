...
if (export_metadata) {
  dir.create(file.path(module_dir, "results", "csv"), showWarnings=FALSE, recursive=TRUE)
  
  # Cell metadata to export (ignore categories based on sketch assay)
  categories_to_export = colnames(sc[[]])
  categories_to_export = categories_to_export[!grepl("\\.sketch$", categories_to_export)]
  categories_to_export = categories_to_export[categories_to_export != "orig_barcode"]
  
  # Embeddings to export (ignore embeddings that were done based on sketch assays)
  embeddings_to_export = purrr::discard(SeuratObject::Reductions(sc), function(r) {
  ...
  for(smp in levels(sc$orig.ident)) {
    dir.create(file.path(module_dir, "results", "csv", smp), showWarnings=FALSE, recursive=TRUE)
    ...
    readr::write_csv(x=categorial_data, file=file.path(module_dir, "results", "csv", smp, "Cell_metadata.csv.gz"))
    ...
    for( r in embeddings_to_export) {
      ...
      readr::write_csv(x=emb,
                file=file.path(module_dir, "results", "csv", smp, paste0("Cell_projection_", r, ".csv.gz")))
    }
  }
  ...
  CalloutBox("Cell metadata can be imported into other tools (such as an active Loupe Cell Browser session). The directory 'csv/datasets' contains files for the individual datasets. The directory 'csv/all' contains files for the aggregated analysis.", type="note")
} else {
...
if (create_loupe_file & all(grepl("10x", sc$technology))) {
  ...
  # Assays to export (currently only default)
  assays_to_export = SeuratObject::DefaultAssay(sc)
  ...
  CalloutBox("Creating loupe files was deactivated or is supported only for datasets of technology '10x', '10x_xenium' or '10x_visium'.", type="note")
}
```
...
if (export_xenium_analysis_file & all(grepl("10x_xenium", sc$technology))) {
  ...
  ExportXeniumExplorer(sc, 
                       categories=categories_to_export, 
                       output_dir=file.path(module_dir, "results", "xenium"), 
                       output_name="analysis.zarr.zip")
  
  CalloutBox("Cell metadata was exported per dataset as analysis.zarr.zip file in the 'xenium' directory. Replace the original analysis.zarr.zip in the respective Xenium dataset (make a backup) to include the results of this analysis in the Xenium Explorer.", type="note")
} else {
  CalloutBox("Exporting to Xenium Explorer was deactivated or is supported only for datasets of technology '10x_xenium'.", type="note")
}
```
