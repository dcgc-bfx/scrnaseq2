...
# sc = SeuratObject::LoadSeuratRds("modules/3_normalization/rna/sc/sc.rds")
...
qc_features = paste(c("nCount", "nFeature", "pMito", "pRibosomal", "pGlobin", "pERCC", "pXIST", "pChrY"), default_assay, sep="_")
if ("DoubletScore" %in% colnames(sc[[]])) qc_features = c(qc_features, "DoubletScore")
...
plist2 = lapply(qc_features, function(x) {
  ggplot(sc[[]], aes(x=seurat_clusters, y=!!sym(x), fill=seurat_clusters, group=seurat_clusters)) + 
...
\`\`\`{r}
#| label: fig-clusterqc_rna_umap_{{type}}
#| fig-cap: '{{caption}}'
...
```{r}
#| label: clusterqc_rna_save_seurat
