...
if (length(qc_features) > 0) {
  ...
  for (i in seq(plist1)) {
    ...
    if (grepl(pattern="nCount", q)) {
        plist1[[i]] = plist1[[i]] + viridis::scale_color_viridis(trans="log10")
        plist1[[i]] = plist1[[i]] + AddPlotStyle(title=paste(q, "(log10)"), xlab="", ylab="")
    } else {
        plist1[[i]] = plist1[[i]] + viridis::scale_color_viridis()
        plist1[[i]] = plist1[[i]] + AddPlotStyle(title=q, xlab="", ylab="")
    }
    
    plist1[[i]] = Seurat::LabelClusters(plist1[[i]], id="ident", box=TRUE, fill="white", label.padding=0.05)
  }
  ...
  plist2 = lapply(qc_features, function(x) {
    p = ggplot(sc[[]], aes(x=seurat_clusters, y=!!sym(x), fill=seurat_clusters, group=seurat_clusters)) + 
      ...
      AddPlotStyle(fill=ScColours(sc, "seurat_clusters"), xlab="Cluster", legend_position="none", title="") +
      theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1))
    
    if (grepl(pattern="nCount", x)) p = p + scale_y_log10()
    return(p)
  })
...
plist[["Phase"]] = LabelClusters(plist[["Phase"]], id="ident", box=TRUE, fill="white", colour="black", label.padding=0.05)
...
for (i in c("S.Score", "G2M.Score", "CC.Difference")) {
  ...
  plist[[i]] = LabelClusters(plist[[i]], id="ident", box=TRUE, fill="white", colour="black", label.padding=0.05)
}
