...
if (use_sketching) {
  ...
  full_reduct = paste0(original_reduct, "_full")
...
for(dim in 1:min(min(20, dim_n_compute), ncells)) {
  dim_feature = paste0(SeuratObject::Key(sc[[reduct]]), dim)
...
ncells = length(Cells(sc[[default_assay]])) 
p = Seurat::ElbowPlot(sc, reduction=DefaultReduct(sc), ndims=min(dim_n_compute, ncells)) + 
  ...
  for(method in integration_methods) {
    ...
    if (use_sketching) {
      ...
      sc = Seurat::ProjectIntegration(sc,
                                      ...
                                      reduction.key=paste0(stringr::str_to_title(full_reduct), "_"),
    ...
    if (is.null(first_reduct)) {
      ...
      if (use_sketching) {
        DefaultReduct(sc) = paste0(first_reduct, "_full")
      }  
    }
  }
...
if (grepl(pattern="_SCT", x=default_assay)) {
    sc = Seurat::PrepSCTFindMarkers(sc, verbose=verbose, assay=default_assay)
    
    # If sketching was used, also apply to sketch
    if (use_sketching) {
        sketched_assay_name = paste(Seurat::DefaultAssay(sc), "sketch", sep=".")
        sc = Seurat::PrepSCTFindMarkers(sc, verbose=verbose, assay=sketched_assay_name)
    }
}
```
...
reductions = dplyr::case_match(integration_methods,
                               ...
                               .default=NULL)
reductions = paste0(default_assay, "_", reductions) %>% tolower()
reductions = c(orig_reduct, reductions)
