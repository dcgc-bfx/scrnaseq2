...
"
  ...
  for(dim in 1:min(min(20, dim_n_compute), ncells)) {
    ...
    plist = FeaturePlotSpatial(sc, images=SeuratObject::Images(sc), features=dim_feature, combine=FALSE) %>% purrr::flatten()
    plist = purrr::map(plist, function(p) {
      p =  p + theme(legend.title=element_blank())
      p = p + viridis::scale_fill_viridis()
      return(p)
    })
    names(plist) = SeuratObject::Images(sc) %>% gsub("^image\\.", "", .)
