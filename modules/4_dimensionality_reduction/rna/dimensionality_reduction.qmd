...
if (use_sketching) {
  ...
  full_reduct = paste(original_reduct, "full", sep=".")
...
Methods might perform differently depending on the dataset and the biological question. We can use multiple methods and compare the results. Please also see: https://www.nature.com/articles/s41592-021-01336-8.
...
if (length(integration_methods) > 0 & num_datasets > 0) {
  ...
  for(method in integration_methods) {
    ...
    if (is.null(first_reduct)) {
      ...
      if (use_sketching) {
        DefaultReduct(sc, assay=default_assay) = paste(first_reduct, "full", sep=".")
      }  
    }
  }
...
```{r}
#| label: integration_rna_integration_plots
#| results: asis
#| eval: !expr length(integration_methods) > 0 & num_datasets > 0
#| include: !expr length(integration_methods) > 0 & num_datasets > 0
#| echo: false
...
chunk_template = "
\`\`\`{r}
#| label: fig-integration_rna_integration_{{type}}
#| fig-cap: '{{caption}}'
#| fig-width: 4
#| fig-height: 4
#| echo: false
