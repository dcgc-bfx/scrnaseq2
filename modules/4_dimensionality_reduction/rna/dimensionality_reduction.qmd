...
reduct_name = paste0(SeuratObject::DefaultAssay(sc), "_", dimensionality_reduction_method) %>% tolower()
...
plist = Seurat::VizDimLoadings(sc, 
                               reduction=ifelse(use_sketching, sketched_reduct, full_reduct), 
...
p = Seurat::ElbowPlot(sc, reduction=ifelse(use_sketching, sketched_reduct, full_reduct), ndims=min(dim_n_compute, ncells)) + 
...
if (length(integration_methods) > 0 & num_datasets > 1) {
  ...
  # Integrate
  orig_reduct = ifelse(use_sketching, sketched_reduct, full_reduct)
  first_method = NULL
  for(method in integration_methods) {
    # Set integrated reduction name, keep track of first method
    ...
    integrated_reduct = paste0(SeuratObject::DefaultAssay(sc), "_", method_short) %>% tolower()
    ...
    if (use_sketching) {
      # Name of sketched integrated reduction
      sketched_reduct = integrated_reduct
      
      # Name and key of full integrated reduction
      full_reduct = gsub("[^_]+_", "", sketched_reduct)
      full_reduct = paste0(tolower(default_assay), "_", full_reduct)
  ...
  DefaultReduct(sc, assay=default_assay) = paste0(tolower(default_assay), "_", first_method)
  if (use_sketching) {
    DefaultReduct(sc, assay=sketched_assay) = paste0(tolower(sketched_assay), "_", first_method)
  } 
...
if (length(integration_methods) > 0 & num_datasets > 1 & use_sketching) {
  merge_method = paste0(tolower(default_assay), "sketch", "_", tolower(dimensionality_reduction_method))
...
if (length(integration_methods) > 0 & num_datasets > 1) {
  merge_method = paste0(tolower(default_assay), "_", tolower(dimensionality_reduction_method))
