...
sc = RunDimRedWrapper(sc, 
                      ...
                      name=ifelse(use_sketching,
                                  paste0(dimensionality_reduction_method, ".sketch"),
...
if (use_sketching) {
  # Name of sketched reduction
  sketched_reduct = paste0(dimensionality_reduction_method, ".sketch")
  
  # Project to full dataset if sketching was used
  full_reduct = dimensionality_reduction_method
  ...
  # Fix titles in the misc slot of the reduction
  sc[[sketched_reduct]]@misc$title = paste0(dimensionality_reduction_method, " (sketch)")
  sc[[full_reduct]]@misc$title = dimensionality_reduction_method
...
#| fig-cap: !expr paste0("Loadings of the dimensionality reduction", ifelse(use_sketching, " for the sketch reduction", ""))
#| fig-height: 4

# When sketching is used, not available for the full dataset

# Plot loadings for first and second dimension
plist = Seurat::VizDimLoadings(sc, dims=1:2, combine=FALSE, balanced=TRUE)
for (i in seq(plist)) {
  plist[[i]] = plist[[i]] + 
    AddPlotStyle()
}
...
#| fig-cap: !expr paste0("Barcodes arranged by the first two dimensions", ifelse(use_sketching, " for the sketch reduction", ""))
...
#| fig-cap: "Barcodes arranged by the first two dimensions projected for the full reduction"
...
#| fig-cap: !expr paste0("Top gene loadings of the first dimensions", ifelse(use_sketching, " for the sketch reduction", ""))
#| fig-subcap: ""
#| echo: false
#| layout-ncol: 3
#| fig-height: 3
...
if (length(SeuratObject::Images(sc)) > 0) {
  # When sketching is used, show only for the full dataset
  
  key = SeuratObject::Key(sc[[DefaultReduct(sc, assay=default_assay)]])
  dim_features = paste0(key, 1:min(min(20, dim_n_compute), ncol(sc)))
...
#| fig-cap: !expr paste0("Variance explained by computed dimensions", ifelse(use_sketching, " for the sketch reduction", ""))
...
p = Seurat::ElbowPlot(sc, ndims=min(dim_n_compute, ncol(sc))) + 
...
if (length(integration_methods) > 0 & num_datasets > 1) {
  ...
  for(method in integration_methods) {
    ...
    if (use_sketching) new_reduct = paste0(new_reduct, ".sketch")
    ...
    # Keep track of the first integrated reduction so that it can be set as default later
    if (is.null(first_reduct)) first_reduct = DefaultReduct(sc)
  }
  
  # Set default reduction
  DefaultReduct(sc, assay=default_assay) = first_reduct
  if (use_sketching) {
    DefaultReduct(sc, assay=sketched_assay) = paste0(first_reduct, ".sketch")
  } 
...
if (length(integration_methods) > 0 & num_datasets > 1 & use_sketching) {
  # Plot different integration methods for the sketched assay
  reductions = dplyr::case_match(integration_methods,
                                     ...
                                     "FastMNNIntegration" ~ "mnn",
                                     "scVIIntegration" ~ "scvii")
  reductions = paste0(reductions, ".sketch")
...
```{r}
#| label: dimensionality_reduction_rna_integration_plots_full
#| results: asis
...
openxlsx::write.xlsx(x=reduct, file=file.path(module_dir, "results", outfile), rowNames=TRUE)
