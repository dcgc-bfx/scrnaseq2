...
params:
  ...
  # test: Type of test for identifying differentially expressed genes. Can be 'wilcox', 'bimod', 'roc', 't', 'negbinom', 'poisson', 'LR', 'MAST', 'DESeq2' (default: 'wilcox').
  ...
  # User-defined genesets in Excel or CSV to use instead MSigDB  The files should have these columns: gs_cat, gs_subcat, gene_id, gene_symbol.
...
# Note: Each list entry is a list either of length 1 (if there is only one comparison) or of length >1 (if multiple subsets are analysed).
contrasts_list = NewContrastsList(sc, contrasts_list)

# For each contrast, prepare an individual Seurat object(s - if multiple subsets analysed) with counts and data. 
# Bulk-aggregate if requested. Downsample if requested.
contrasts_list = purrr::map_depth(contrasts_list, 2, function(c) return(PrepareContrast(sc)
...
if (is.logical(run_ora)) {
    ...
    deg_ora_idx = seq(contrasts_list) %>% 
      purrr::keep(function(i) return(contrasts_list[[i]]$name %in% run_ora))
}
...
if (have_deg_comparisons) {
    ...
    deg_results_list = purrr::map(deg_results_list, function(deg_result) {
    ...
    deg_results_list = split(deg_results_list,
                             purrr::map_chr(deg_results_list, function(deg_result) return(deg_result$name)) )
    ...
    for (i in seq_along(deg_results_list)) {
      ...
      # Write to file
      outfile = paste0("degs_", deg_results[[1]]$name, "_results.xlsx")
...
\`\`\`{r}
#| label: tbl-deg_analysis_results_{{name}}
#| tbl-cap: DEG results for comparison
...
"
        ...
        if ("subset_column" %in% names(deg_results[[1]])) {
          ...
          subset_groups = purrr::map_chr(deg_results, function(deg_result) return(paste(deg_result$subset_group, collapse="+"))) %>% paste(collapse=", ")
        } else {
          subset_column = NULL
          subset_groups = NULL
        }
        ...
        chunk_filled = knitr::knit_expand(text=chunk_template_overview,
                                          ...
                                          subset_group=subset_groups %||% "-",
                                          bulk_by=deg_results[[1]]$bulk_by %||% "-",
                                          pseudobulk_samples=deg_results[[1]]$pseudobulk_samples %||% 0,
                                          assay=deg_results[[1]]$assay,
                                          ...
                                          downsample_n=deg_results[[1]]$downsample_n %||% 0,
        ...
        for (j in seq_along(deg_results)) {
            ...
            if ("subset_column" %in% names(deg_result)) {
              ...
              cat(FormatString("#### Subset {subset_column}: {subset_group}"), '\n\n')
            } else {
              subset_column = ""
              subset_group = ""
            }
...
\`\`\`{r}
#| label: tbl-deg_analysis_results_{{name}}_{{j}}
#| tbl-cap: \"Top five genes tested for comparison\"
...
#| fig-cap: \"Scatter and volcano plot for comparison\"
#| fig-height: 4
...
"
            ...
            chunk_filled =  knitr::knit_expand(text=chunk_template_comparison, 
                                               name=name,
                                               name_subset=ifelse(nchar(subset_group)>0, paste0(name,", ", subset_column, ": ", subset_group), name),
                                               j=j)
            if(interactive()) {
                print(EvalKnitrChunk(chunk_filled))
            } else {
