...
if (have_deg_comparisons) {
    # Generate chunk templates
    
    # 1. for the overview part (at the beginning of the contrast section)
...
#| tbl-cap: Results for comparison {{name}}
...
\`\`\`
"
    # 2. for the results(s) obtained for the contrast (can be more if multiple subsets were analysed)
...
"
    ...
    for (deg_results in deg_results_list) {
        ...
        # Now iterate over the result(s) for this comparison. If multiple subsets were analysed, there will be multiple results.
