...
if (have_deg_comparisons) {
    ...
    for (i in seq_along(deg_results_list)) {
      ...
      config_values = ifelse(config_names %in% names(deg_results[[1]]), deg_results[[1]][config_names], NA) %>% unlist()
      if ("subset_column" %in% names(deg_results[[1]])) {
...
#| tbl-cap: Top five genes tested for comparison '{{name_subset}}'
...
#| fig-cap: Scatter and volcano plot for comparison '{{name_subset}}'
#| fig-height: 4
...
"
            ...
            chunk_filled =  knitr::knit_expand(text=chunk_template_comparison, 
                                               name=name,
                                               name_subset=ifelse(nchar(subset_group)>0, paste0(name,": ", subset_group), name),
                                               j=j)
            if(interactive()) {
                print(EvalKnitrChunk(chunk_filled))
            } else {
