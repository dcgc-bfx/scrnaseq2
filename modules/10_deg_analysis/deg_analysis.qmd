...
params:
  ...
  #       The following tests can use covariates: 'LR', 'poisson', 'negbinom', 'MAST', 'DESeq2'.
  # padj: Maximum adjusted p-value (default: 0.05)
  # log2FC: Minimum absolute log2 fold change (default: 0)
  #         Only used when testing counts data.
  ...
  # When using MSigDB, the name of the species in the database. If null, the name will be inferred from the 'species' parameter. For example: homo_sapiens => Homo sapiens
  msigdb_species: null
...
if (have_deg_comparisons) {
    ...
    for (i in seq_along(deg_results_list)) {
      ...
      config_values = purrr::map_chr(config_names, function(n) {
        v = deg_results[[1]][[n]] %||% "-"
        v = paste(v, collapse=ifelse(n %in% c("condition_group1", "condition_group2"), "+", ","))
        return(v)
      })
