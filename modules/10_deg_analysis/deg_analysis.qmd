...
deg_summary_table = purrr::map_dfr(deg_results_list, function(result) {
...
gt::gt(deg_summary_table) %>%
    gt::sub_missing(columns = 1:ncol(deg_summary_table),
    missing_text = ""
  )
```

```{r}
#| label: deg_analysis_generate_reports
#| results: asis
#| eval: !expr have_deg_comparisons
#| include: !expr have_deg_comparisons
...
if (have_deg_comparisons) {
    for(i in seq(deg_results_list)) {
        result = deg_results_list[[i]]
        DegsGenerateReport(result, i)
    }
