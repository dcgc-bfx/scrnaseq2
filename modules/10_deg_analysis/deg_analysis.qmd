...
if (is.character(param("deg_contrasts")) && file.exists(param("deg_contrasts"))) {
  # Read from file
  deg_contrasts_file = param("deg_contrasts")
  ...
  deg_contrasts_list = readxl::read_excel(deg_contrasts_file, sheet=1, col_names=TRUE)
  deg_contrasts_list = split(deg_contrasts_list, seq(nrow(deg_contrasts)))
  ...
  deg_contrasts_list = param("deg_contrasts")
    
  # Read from yaml parameter and prepare list
  deg_contrasts_list = purrr::map(seq(deg_contrasts_list), function(i) {
      contrast = deg_contrasts_list[[i]][[1]]
      contrast$name = names(deg_contrasts_list[[i]])
      return(contrast)
  })
}

# Check if contrasts are set up correctly, 
deg_contrasts_list = SetupContrastsList(sc, deg_contrasts_list)

# Set up objects
deg_contrasts = PrepareDegContrasts(sc, deg_contrasts_list)
