...
if (have_deg_comparisons) {
    ...
    for (i in seq_along(deg_results_list)) {
      ...
      config_values = purrr::map_chr(config_names, function(n) {
        v = as.character(deg_results[[1]][[n]] %||% "-")
        if (n %in% c("condition1", "condition2")) v = paste(v, collapse="+")
        if (n %in% "subset_groups") {
            v = purrr::map_chr(deg_results, function(deg_result) return(paste(deg_result$subset_group, collapse="+"))) %>% 
                paste(collapse=", ")
        }
        return(paste(v, collapse=","))
      })
      config_desc = c("condition column", "condition group 1", "condition group 2", "subset column", "subset groups",
                     "bulk by", "pseudobulk samples", "assay", "data", "test", "maximum adjusted p-value",
                     "minimum absolute log2 foldchange", "minimum percentage of expressing cells", "downsample n", "covariate", "design formula", "DESeq2 results arguments")

      deg_configuration_table = data.frame(Config=config_names, Desc=config_desc, Value=config_values)
      rownames(deg_configuration_table) = NULL
...
if (have_ora) {
  ...
  ora_results_list = purrr::map_depth(deg_results_list, .depth=2, function(deg_result) {
    # Is ORA requested for this DEG analysis?
    name = deg_result[["name"]]
    ora_result = list(name=name)
    if (name %in% run_ora) {
      # Run ORA and add results
      ora_result[["results"]] = DegsRunOraTest(deg_result=deg_result,
                               term2gene_db=ora_term2gene_db,
                               genesets=deg_result[["ora_genesets"]] %||% param("genesets"))
      
      # Also create bar plots with the top 15 terms
      ora_result[["plots"]] = purrr::map(ora_result[["results"]], function(ora) {
          # If there are no results, return NULL
          if (nrow(ora) == 0) return(NULL)
          
          p = enrichplot:::dotplot(ora, color='p.adjust', x='FoldEnrichment', size='Count', font.size=11, showCategory=15, label_format=50) +
              AddPlotStyle() +
      ...
      # Else just add an empty list
      ora_result[["results"]] = list()
      ora_result[["plots"]] = list()
    }
    
    return(ora_result)
  })
  ora_results_list = ora_results_list[run_ora]
...
if (have_ora) {
  ...
  for (ora_results in ora_results_list) {
    ...
    for (i in seq_along(ora_results)) {
      # deg_result: DEG analysis for this subset comparison
      ora_result = ora_results[[i]]
      name = ora_result$name
      
      # ora_result: ORA analysis for this subset comparison
      ora_result = ora_result[["results"]]
