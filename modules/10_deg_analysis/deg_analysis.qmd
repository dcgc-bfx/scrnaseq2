...
params:
  ...
  #       Tests 'DESeq2' and 'DESeq2LRT' can only be run with  'bulk_by'/'pseudobulk_samples'. 
  ...
  # User-defined genesets in Excel or CSV file(s) to use for ORA instead of MSigDB. The files should have these columns: gs_cat,
...
contrasts_list = NewContrastsList(sc, contrasts_list, type="deg")
...
contrasts_list = purrr::map_depth(contrasts_list, .depth=2, function(contrast) return(PrepareDegContrast(sc, contrast)))
...
if (have_deg_comparisons) {
    ...
    for (i in seq_along(deg_results_list)) {
      ...
      config_names = c("condition_column", "condition1", "condition2", "bulk_by", "pseudobulk_samples", "assay", "test", "padj", "log2FC", "min_pct", "covariate", "formula", "downsample_n")
      config_values = purrr::map_chr(config_names, function(n) {
        v = deg_results[[1]][[n]] %||% "-"
        v = paste(v, collapse=ifelse(n %in% c("condition1", "condition2"), "+", ","))
        return(v)
      })
...
deg_summary_table = deg_summary_table %>% 
    dplyr::select(Comparison, Condition, Group1, Group2, Subset, SubsetGroup, Sign, Sign_up, Sign_down)
...
* condition group 1: {{condition1}}
* condition group 2: {{condition2}}
...
* output file(s): 
  * DEGs: degs_{{name}}.xlsx
  * Overrepresentation analysis (if requested): degs_{{name}}_ora.xlsx
  * Figures: 
    * fig-deg_analysis_deg_plots_{{name}}_SUBSET-1.png/pdf
    * fig-deg_analysis_deg_ora_{{name}}_SUBSET_ANALYSIS-1.png/pdf
"
...
deg_results_table = deg_summary_table %>% 
    ...
    dplyr::select(Condition, Group1, Group2, Subset, SubsetGroup, Sign, Sign_up, Sign_down)
    
gt::gt(deg_results_table) %>%
    gt::sub_missing(columns=1:ncol(deg_results_table), missing_text='') %>%
    gt::fmt_number(columns=2:ncol(deg_results_table), decimals=3, use_seps=FALSE)
\`\`\`
...
enrichplot:::barplot.enrichResult(ora_res, x='FoldEnrichment', font.size=11, showCategory=10, label_format=40) +
  geom_text(aes(label=Count), hjust=1, size=11/.pt, color='firebrick') +
...
* output file(s): 
  ...
  * Figures: 
    ...
    * fig-deg_analysis_deg_ora_{{name}}_SUBSET_ANALYSIS-1.png/pdf
    ...
    config_values = c(condition_column=deg_results[[1]]$condition_column, condition1=deg_results[[1]]$condition1, condition2=deg_results[[1]]$condition2,
                 subset_column=deg_results[[1]]$subset_column %||% "-", subset_group=names(deg_results) %>% paste(collapse=", "),
                 bulk_by=deg_results[[1]]$bulk_by %||% "-", pseudobulk_samples=deg_results[[1]]$pseudobulk_samples %||% "-",
                 assay=deg_results[[1]]$assay, data=deg_results[[1]]$layer,
                 test=deg_results[[1]]$test, padj=deg_results[[1]]$padj, log2FC=deg_results[[1]]$log2FC, min_pct=deg_results[[1]]$min_pct,
                 covariate=deg_results[[1]]$covariate %||% "-", downsample_n=deg_results[[1]]$downsample_n %||% "-",
                 deg_files=paste0("degs_", deg_results[[1]]$name, ".xlsx"),
                 ora_files=paste0("degs_", deg_results[[1]]$name, "_ora.xlsx"))
    config_names = c("condition column", "condition group 1", "condition group 2", "subset column", "subset groups",
                     "bulk by", "pseudobulk samples", "assay", "data", "test", "maximum adjusted p-value",
                     "minimum absolute log2 foldchange", "minimum percentage of expressing cells", "covariate", "downsample n",)
...
if (have_gsea) {
  ...
  for (gsea_results in gsea_results_list) {
    ...
    for (i in seq_along(gsea_results)) {
      ...
      # Add a small dummy plot that contains the name of the subset (if there is one)
      if ("subset_column" %in% names(gsea_result)) {
        ...
        p = ggplot() + 
          annotate("text", x = 4, y = 25, size=8, label = FormatString("{subset_column}: {subset_group}", quote=FALSE)) + 
          theme_void()
        print(p)
      }
      ...
      for (j in seq_along(gsea_result[["results"]])) {
        ...
        # Add a small dummy plot that contains the name of the GSEA analysis
        p = ggplot() + 
          annotate("text", x = 4, y = 25, size=8, label = FormatString("Geneset: {gsea_res_name}", quote=FALSE)) + 
          theme_void()
        print(p)
...
# Chunk template for the GSEA configuration (per comparison)
...
* downsample n: {{downsample_n}}
* output file(s):
  * Results of GSEA analyses: gsea_{{name}}.xlsx
  * Plots for the top n terms in each GSEA analysis:gsea_{{name}}_plots.pdf
  * Figures: fig-deg_analysis_gsea_plots_{{name}}_SUBSET_ANALYSIS-1.png/pdf
"
...
if (have_gsea) {
  ...
  for (gsea_results in gsea_results_list) {
    ...
    chunk_filled = knitr::knit_expand(text=chunk_template_gsea_configuration,
                                      ...
                                      condition1=gsea_results[[1]]$condition1,
                                      condition2=gsea_results[[1]]$condition2,
