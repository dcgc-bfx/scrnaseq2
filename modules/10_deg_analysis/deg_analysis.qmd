  ...
  contrasts:
      value:
          ...
          - Comparison1:
                ...
                subset_group: "*"
                test: "DESeq2"
                downsample_barcodes: 500
          # Compare BM_LSK_C vs BM_LSK_R
          # - Comparison2:
          ...
          # - Comparison3:
          #       condition_column: "orig.ident"
          #       condition1: "BM_Monocytes_C + SPL_Monocytes_C"
          #       condition2: "BM_Monocytes_R + SPL_Monocytes_R"
...
## removing the integers from the orig.ident
sc@meta.data$orig.ident = as.factor(gsub("[0-9]", "", sc@meta.data$orig.ident))
...
if (have_deg_comparisons) {
  ...
  for (deg_results in deg_results_list) {
    ...
    chunk_filled = knitr::knit_expand(text=chunk_template_configuration,
                                      ...
                                      condition1=deg_results[[1]]$condition1,
                                      condition2=deg_results[[1]]$condition2,
...
if (have_gsea) {
  ...
  for (gsea_results in gsea_results_list) {
    ...
    for (i in seq_along(gsea_results)) {
      ...
      if (is.null(gsea_result$results) || length(gsea_result$results) == 0) next
      ...
      cat("::: {.panel-tabset}\n")
      valid_results = gsea_result[["results"]][sapply(gsea_result[["results"]], function(x) !is.null(x) && nrow(x) > 0)]
      if (length(valid_results) == 0) next
      
        cat("::: {.panel-tabset}\n")
        for (j in seq_along(valid_results)) {
          gsea_res = valid_results[[j]]
