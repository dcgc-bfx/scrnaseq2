...
::: {.callout-tip title="Why is pre-processing so important?" collapse="true"}
...
if (length(SeuratObject::Images(sc)) > 0) {
    ...
    if (length(barcode_qc) > 0) {
        ...
        for(q in barcode_qc) {
            ...
            # Make captions
            captions = GeneratePlotCaptions(q, remove=paste0("_", default_assay))
...
"
        ...
        CalloutBox(x="No barcode QC1 parameters for plotting available.", type="note")
    }
}
```
  ...
  if (species != "homo_sapiens") {
    ...
    seurat_g2m_genes = EnsemblFetchOrthologues(ids=seurat_g2m_genes,
                                          ...
                                          mart_attributes2=c(ensembl_id2="ensembl_gene_id", 
                                                            ensembl_symbol2="external_gene_name"),
                                          useCache=TRUE)
    seurat_g2m_genes = seurat_g2m_genes %>% dplyr::filter(one_to_one == TRUE) %>% dplyr::pull(ensembl_id2)
  } else {
