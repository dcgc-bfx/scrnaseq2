...
if (length(SeuratObject::Images(sc)) > 0) {
    ...
    if (length(barcode_qc) > 0) {
        ...
        for(q in barcode_qc) {
            ...
            # Make captions
            idents = SeuratObject::Images(sc) %>% gsub("^(fov|image|slice)\\.", "", .)
...
if (length(SeuratObject::Images(sc)) > 0) {
  ...
  plist = DimPlotSpatial(sc, 
                         cells.highlight=purrr::flatten(barcodes_to_exclude) %>% purrr::flatten_chr() %>% unique())
  names(plist) = gsub("^(fov|image|slice)\\.", "", names(plist))
  plist = purrr::map(plist, function(p) {
    return(p + scale_fill_manual("Status", labels=c("Filtered", "Ok"), values=c("#DE2D26", "grey60")))
  })
...
if (have_cell_cycle_scores & length(SeuratObject::Images(sc)) > 0) {
