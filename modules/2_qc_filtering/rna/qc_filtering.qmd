...
barcodes_to_exclude = purrr::map(orig_idents, function(s) {
  ...
  # Samples to drop
  samples_to_drop = param("samples_to_drop")
  if (s %in% samples_to_drop) {
...
features_to_exclude_tbl = purrr::map_dfr(names(features_to_exclude), function(n) {
  ...
  df = data.frame(N=length(features), FailThreshold=sum(features))
  
  df$Excluded = length(setdiff(names(features), features_to_exclude_all))
  df$PercKept = round( (df$N - df$FailThreshold) / df$N * 100, 2)
  return(df)
})
