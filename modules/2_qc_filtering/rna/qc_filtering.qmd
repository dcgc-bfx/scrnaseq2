...
# Note: In case the argument 'features' is empty, PercentageFeatureSet will try the argument 'pattern' and will fail since 'pattern' is NULL. 
# We provide a dummy pattern to avoid this. Ignore the warning.
mt_features = grep(pattern="^MT-", x=rownames(feature_metadata), value=TRUE, ignore.case=TRUE)
mt_vals = Seurat::PercentageFeatureSet(sc, features=mt_features, pattern="^$")
mt_vals = ifelse(is.nan(mt_vals), 0, mt_vals)
sc = Seurat::AddMetaData(sc, mt_vals, col.name=paste("pMito", default_assay, sep="_"))
...
ribo_vals = Seurat::PercentageFeatureSet(sc, features=ribo_features, pattern="^$")
ribo_vals = ifelse(is.nan(ribo_vals), 0, ribo_vals)
sc = Seurat::AddMetaData(sc, ribo_vals, col.name=paste("pRibosomal", default_assay, sep="_"))
...
globin_vals = Seurat::PercentageFeatureSet(sc, features=globin_features, pattern="^$")
globin_vals = ifelse(is.nan(globin_vals), 0, globin_vals)
sc = Seurat::AddMetaData(sc, globin_vals, col.name=paste("pGlobin", default_assay, sep="_"))

# Calculate percentage of counts in globin genes
ercc_features = grep("^ERCC-", x=rownames(feature_metadata), value=TRUE, ignore.case=TRUE)
ercc_vals = Seurat::PercentageFeatureSet(sc, features=ercc_features, pattern="^$")
ercc_vals = ifelse(is.nan(ercc_vals), 0, ercc_vals)
sc = Seurat::AddMetaData(sc, ercc_vals, col.name=paste("pERCC", default_assay, sep="_"))

# Calculate number of counts in XIST
xist_feature = grep("^XIST", x=rownames(feature_metadata), value=TRUE, ignore.case=TRUE)
xist_vals = Seurat::PercentageFeatureSet(sc, features=xist_feature, pattern="^$")
xist_vals = ifelse(is.nan(xist_vals), 0, xist_vals)
sc = Seurat::AddMetaData(sc, xist_vals, col.name=paste("pXIST", default_assay, sep="_"))

# Calculate percentage of counts in chrY genes
chr_column_names = c("ensembl_chr", "ensembl_chromosome", "chr", "chromosome", "chrom")
chr_column_names = chr_column_names[chr_column_names %in% tolower(colnames(feature_metadata))]
if (length(chr_column_names) > 0) {
  chr_column_name = chr_column_names[1]
  chrY_features_idx = which(feature_metadata[[chr_column_name]] %in% c("chrY", "Y"))
  chrY_features = rownames(feature_metadata)[chrY_features_idx]
  
  chrY_vals = Seurat::PercentageFeatureSet(sc, features=chrY_features)
  chrY_vals = ifelse(is.nan(chrY_vals), 0, chrY_vals)
} else {
 chrY_features = c()
 chrY_vals = 0
}
sc = Seurat::AddMetaData(sc, chrY_vals, col.name=paste("pChrY", default_assay, sep="_"))
...
# Some related variables we use very often
bcs = SeuratObject::Cells(sc[[default_assay]])
barcode_metadata = sc[[]][bcs,]
orig_idents = levels(barcode_metadata$orig.ident)
feature_metadata = sc[[default_assay]][[]]
```

We start the analysis by removing unwanted barcodes from the dataset(s). Three commonly used QC metrics include: 

- the number of unique features detected in each cell ("nFeature_RNA")
- the total number of molecules detected in each cell ("nCount_RNA"), and
- the percentage of counts that map to the mitochrondrial genome ("percent_mt"). 
...
On the one hand, low-quality cells such as dying, degraded and damaged cells, and empty droplets will often result in few genes ("nFeature_RNA") and low total counts ("nCount_RNA"). On the other hand, cell doublets may show an aberrantly high number of genes. Since the total number of reads detected within a barcode typically strongly correlates with the number of unique features, we can focus on the number of unique features for filtering. In addition, damaged cells often exhibit high mitochondrial ("percent_mt") or spike-in ("percent_ercc") content. As mitochondria have their own membranes, their RNA is often the last to degrade in damaged cells and can thus be found in high numbers. However, it is important to keep in mind that different cell types and cells isolated from various species may differ in their number of expressed genes and metabolism. For example, stem cells may express a higher number of unique genes, and metabolically active cells may express a higher number of mitochondrial genes.

:::

::: {.callout-tip title="Impact of low-quality barcodes on downstream analyses" collapse="true"}

First of all, low-quality barcodes of different cell types might cluster together due to similarities in their damage-induced gene expression profiles. This leads to artificial intermediate states or trajectories between subpopulations that would otherwise be distinct. Second, low-quality barcodes might mask relevant gene expression changes. The main differences captured by the first principal components will likely be based on barcode quality rather than the underlying biology, reducing the power of dimensionality reduction. Likewise, highly variable features might just be features that differ between low- and high-quality barcodes. Lastly and equally important, the low number of total counts in low-quality barcodes might lead to artificial upregulation of features due to wrong scaling effects during the normalisation process.
...
f = barcode_qc %in% colnames(barcode_metadata)
if (!all(f)) {
...
f = purrr::map_lgl(barcode_qc_cor, function(b) return(!any(b %in% missing_barcode_qc)))
...
highestExpr = purrr::map_dfr(SeuratObject::Layers(sc, search="^counts\\."), function(l) {
  n = gsub(x=l, pattern="counts\\.", replacement="")
...
When working with human or animal samples, you should ideally constrain your experiments to a single sex in order to avoid bias. However, this may not always be possible. By looking at reads from chromosome Y (males) and XIST (X-inactive specific transcript) expression (mainly female) it is possible to determine sample sex. 
...
sum_xist = sum(barcode_metadata[, paste("pXIST", default_assay, sep="_")])
sum_chry = sum(barcode_metadata[, paste("pChrY", default_assay, sep="_")])
...
The number of excluded cells is as follows:
...
```{r}
#| label: qc_filtering_filter_barcodes_spatial
#| eval: !expr length(image_names) > 0
#| include: !expr length(image_names) > 0
#| results: asis

if (length(image_names) > 0) {
  # Plot as filtered barcodes as spatial plots
  cat("\nThe filtered cells are shown in red in the spatial plots below.\n\n")

  # Add a cell metadata column to colour by
  barcodes_to_exclude_flat = purrr::flatten(barcodes_to_exclude) %>% purrr::flatten_chr() %>% unique()
  sc$cells.highlight = ifelse(rownames(sc[[]]) %in% barcodes_to_exclude_flat, "Filtered", "Ok")
  sc$cells.highlight = factor(sc$cells.highlight, levels=c("Ok", "Filtered"))
  
  plist = DimPlotSpatial(sc,
                         images=image_names,
                         group.by="cells.highlight")
  plist = purrr::map(plist, function(p) {
    return(p + scale_fill_manual("Status", values=c("Filtered"="#DE2D26", "Ok"="grey60")))
  })
  names(plist) = gsub("^image\\.", "", image_names)

  # Set up layout for generating chunks
  chunk_template = "
\`\`\`{r}
#| label: fig-qc_filtering_filter_barcodes_spatial_dataset_{{dataset}}
#| fig-cap: {{caption}}
#| dev: png
...
"
  ...
  for(dataset in names(plist)) {
    cat ("#####", dataset, "\n\n")
    chunk_filled =  knitr::knit_expand(text=chunk_template, dataset=dataset, caption=FormatString("Filtered cells for dataset {dataset}"))
    if(interactive()) {
      print(EvalKnitrChunk(chunk_filled))
    } else {
        chunk_filled = knitr::knit_child(text=chunk_filled, envir=environment(), quiet=TRUE)
  ...
  # Remove column
  sc$cells.highlight = NULL
}
```

```{r}
#| label: qc_filtering_rna_filter_barcodes
...
barcodes_to_exclude_assay = setdiff(barcodes_to_exclude, barcodes_other_assays)
barcodes_to_keep = setdiff(SeuratObject::Cells(sc[[default_assay]]), barcodes_to_exclude_assay)
...
barcodes_to_exclude_sc = setdiff(barcodes_to_exclude, barcodes_other_assays)
if (length(barcodes_to_exclude_assay) > 0) {
  barcodes_sc = purrr::map(SeuratObject::Assays(sc), function(a) return(SeuratObject::Cells(sc[[a]]))) %>% purrr::flatten_chr() %>% unique()
  barcodes_sc = setdiff(barcodes_sc, barcodes_to_exclude_sc)
...
The number of excluded features is as follows:
...
barcode_filter = PrepareBarcodeFilter(param("barcode_filter"), orig_idents, sc[[]])
...
# Species, Ensembl version (if used)
species = param("species")
ensembl_version = param("ensembl")

# Barcode and feature metadata
barcode_metadata = sc[[]]
feature_metadata = sc[[default_assay]][[]]
...
# Find the column with ensembl ids and keep if it contains ensembl ids
ensembl_id_columns = c("ensembl_id", "ensembl_gene_id", "gene_id", "feature_id")
ensembl_id_columns = ensembl_id_columns[ensembl_id_columns %in% tolower(colnames(feature_metadata))]
ensembl_id_columns = purrr::keep(ensembl_id_columns, function(c) {
  any(grepl("^ENS[A-Z]+[0-9]+$", feature_metadata[[c]]))
})

if (length(ensembl_id_columns) > 0) {
  ensembl_column = ensembl_id_columns[1]
} else {
  ensembl_column = NULL
}
...
# Update barcode_metadata variable
bcs = SeuratObject::Cells(sc[[default_assay]])
barcode_metadata = sc[[]][bcs,]
```
...
#| eval: !expr have_cell_cycle_scores & length(image_names) > 0
#| include: !expr have_cell_cycle_scores & length(image_names) > 0
#| results: asis


if (have_cell_cycle_scores & length(image_names) > 0) {
  # Plot as spatial plots
  plist = DimPlotSpatial(sc, images=image_names, group.by="Phase", combine=FALSE, cols=ScColours(sc, "Phase")) %>% purrr::flatten()
...
if (length(cell_cycle_markers) > 0) {
  ...
  plist = Seurat::VlnPlot(sc, features=cell_cycle_markers, combine=FALSE, pt.size=0, assay=default_assay,layer="data")
  plist = purrr::map(plist, function(p) {
...
\`\`\`
"
  chunk_filled =  knitr::knit_expand(text=chunk_template, caption="Expression of common cell-cycle markers")
  if(interactive()) {
    print(EvalKnitrChunk(chunk_filled))
  } else {
    chunk_filled = knitr::knit_child(text=chunk_filled, envir=environment(), quiet=TRUE)
...
"
    ...
    for(gene in cell_cycle_markers) {
      ...
      # Plot as spatial plots
      plist = FeaturePlotSpatial(sc, images=image_names, features=gene, combine=FALSE) %>% purrr::flatten()
      plist = purrr::map(plist, function(p) {
        p =  p + theme(legend.title=element_blank())
...
# Write metadata
outfile = FormatString("cell_metadata_qc_assay_{default_assay}.xlsx", quote=FALSE)
openxlsx::write.xlsx(x=sc[[]], file=file.path(module_dir, "results", outfile), rowNames=TRUE)
