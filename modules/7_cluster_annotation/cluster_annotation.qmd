...
if (markers_found) {
  ...
  # Show table
  gt(markers_top %>% dplyr::select(-labels)) %>% 
...
if (markers_found) {
  ...
  for (cl in markers_top$cluster %>% levels()) {
    ...
    plist[["FeaturePlot"]][[cl]] = lapply(markers_top_cl$gene, function(x) {
        p = Seurat::FeaturePlot(sc, features=x, order=TRUE) + 
            viridis::scale_color_viridis() + 
            AddPlotStyle(legend_position="right", xlab="", ylab="")
        p = Seurat::LabelClusters(p, id="ident", box=TRUE, fill="white", colour="black", label.padding=0.05)
        return(p)
    })
    ...
    plist[["ViolinPlot"]][[cl]] = lapply(markers_top_cl$gene, function(x) { 
        Seurat::VlnPlot(sc, features=x, assay=default_assay, pt.size=0, cols=ScColours(sc, "seurat_clusters"), flip=TRUE) + 
            AddPlotStyle(legend_position="none", xlab="Cluster", ylab="") +
  ...
  # Dotplot (unscaled)
  plist[["DotPlotNonscaled"]] = Seurat::DotPlot(sc, features=markers_top$gene %>% unique(), scale=FALSE) + 
    viridis::scale_color_viridis() + 
    AddPlotStyle(ylab="Cluster", legend_position="bottom") + 
    theme(axis.text.x = element_text(angle=90, hjust=1, vjust=.5)) + 
    guides(size=guide_legend(order=1))
...
if ((known_markers_n > 0) & length(known_markers_vect) <= 100) {
  plist = purrr::map(known_markers_list, function(f) {
    pl = Seurat::FeaturePlot(sc, features=f, combine=FALSE, order=TRUE)
    for (i in seq(pl)) {
      pl[[i]] = pl[[i]] + viridis::scale_color_viridis() + AddPlotStyle(xlab="", ylab="")
      pl[[i]] = Seurat::LabelClusters(pl[[i]], id="ident", box=TRUE, fill="white", colour="black", label.padding=0.05)
    }
    names(pl) = f
    return(pl)
  })
  names(plist) = names(known_markers_list)
  
  # Set up layout and automatically generate chunks
  chunk_template = "
\`\`\`{r}
#| label: fig-cluster_annotation_knownmarkers_featureplots_{{gene}}
#| fig-cap: UMAP coloured by expression of {{gene}}
#| echo: false
#| fig-width: 4
#| fig-height: 3
...
"
  ...
  for (lst in names(plist)) {
    ...
    # Loop through genes
    for (gene in names(plist[[lst]])) {
      chunk_filled =  knitr::knit_expand(text=chunk_template, gene=gene, lst=lst)
      if(interactive()) {
        print(EvalKnitrChunk(chunk_filled))
      } else {
        chunk_filled = knitr::knit_child(text=chunk_filled, envir=environment(), quiet=TRUE)
