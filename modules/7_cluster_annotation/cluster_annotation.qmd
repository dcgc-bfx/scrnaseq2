...
markers_list = lapply(SeuratObject::Assays(sc), function(assay) {
  markers_intermediate = SeuratWrappers::RunPrestoAll(object=sc,
                                       assay=assay,
  ...
  if (ncol(markers_intermediate)==0) markers_intermediate = DegsEmptyMarkerResultsTable(levels(barcode_metadata[, cluster_column]))
  
  # Sort markers (function "DegsSort" in "R/functions_degs.R")
  markers_intermediate = markers_intermediate %>% DegsSort(group=c("cluster"))
    
  # Filter markers (function "DegsFilter" in "R/functions_degs.R")
  markers_filt_intermediate = DegsFilter(markers_intermediate, cut_log2FC=param("marker_log2FC"))
  
  # Add average data to table (function "DegsAvgDataPerIdentity" in "R/functions_degs.R")
  markers_out_intermediate = cbind(markers_filt_intermediate$all, DegsAvgDataPerIdentity(sc, genes=markers_filt_intermediate$all$gene, assay=assay))
  
  # Add marker table to Seurat object
  Seurat::Misc(sc, paste0("markers_", assay)) = list(condition_column=cluster_column, test="wilcox", 
                                     log2FC=param("marker_log2FC"), 
                                     min_pct=param("marker_pct"), 
                                     assay=default_assay, 
                                     layer="data",
                                     latent_vars=param("latent_vars"),
                                     results=markers_filt_intermediate$all)

  markers_filt_intermediate$all$gene = paste0(assay, "_", markers_filt_intermediate$all$gene)
  markers_filt_intermediate$up$gene = paste0(assay, "_", markers_filt_intermediate$up$gene)
  markers_filt_intermediate$down$gene = paste0(assay, "_", markers_filt_intermediate$down$gene)
  
  # return(markers_filt_intermediate)
  return(list(
    filtered = markers_filt_intermediate,
    with_avg = markers_out_intermediate
  ))
})

# Note that markers_out is written to file at the end of the module
markers_out = map(markers_list, "with_avg") %>% bind_rows()

markers_filt = list(
  all  = map(markers_list, ~ .x$filtered$all)  %>% bind_rows(),
  up   = map(markers_list, ~ .x$filtered$up)   %>% bind_rows(),
  down = map(markers_list, ~ .x$filtered$down) %>% bind_rows()
)
...
CalloutBox("The prefix (RNA_, ADT_, or else) to the marker names signify what assay they come from.", type="note")
```
...
"
    ...
    chunk_template2 = "
\`\`\`{r}
#| label: fig-cluster_annotation_markers_{{type}}_rna
#| fig-cap: {{caption}}
...
"
    ...
    for (type in c("FeaturePlot", "ViolinPlot")) {
        ...
        for (cl in markers_top$cluster %>% levels()) {
            ...
            valid_genes = gsub("RNA_", "", markers_top_cl$gene[gsub("RNA_", "", markers_top_cl$gene) %in% rownames(sc[["RNA"]])])
            valid_proteins = gsub("ADT_", "", markers_top_cl$gene[gsub("ADT_", "", markers_top_cl$gene) %in% rownames(sc[["ADT"]])])

            # Produce plots
            if (type == "FeaturePlot") {
              plist_rna = plist_adt = list()
              if(length(valid_genes)>0) {
                  DefaultAssay(sc) = "RNA"
                  plist_rna = Seurat::FeaturePlot(sc, features=valid_genes, reduction="umap", order=TRUE, combine=FALSE)
                  plist_rna = lapply(plist_rna, function(p) {
                    p$labels$title = paste0(p$labels$title, "_RNA")
                    return(p)
                  })
                }
                ...
                if(length(valid_proteins)>0){
                  DefaultAssay(sc) = "ADT"
                  plist_adt = Seurat::FeaturePlot(sc, features=valid_proteins, reduction="umap", order=TRUE, combine=FALSE)
                  
                  plist_adt = lapply(plist_adt, function(p) {
                    p$labels$title = paste0(p$labels$title, "_ADT")
                    return(p)
                  })
                }
              
                plist = c(plist_rna, plist_adt)
                ...
                if(length(valid_genes)>0) {
                  plist_rna = Seurat::VlnPlot(sc, features=valid_genes, assay="RNA", pt.size=0, cols=ScColours(sc, "seurat_clusters"), flip=TRUE, combine=FALSE)
                ...
                if(length(valid_proteins)>0){
                  plist_adt = Seurat::VlnPlot(sc, features=valid_proteins, assay="ADT", pt.size=0, cols=ScColours(sc, "seurat_clusters"), flip=TRUE, combine=FALSE)
    ...
    for (type in c("DotPlotScaled", "DotPlotNonscaled")) {
        ...
        valid_genes = gsub("RNA_", "", markers_top$gene[gsub("RNA_", "", markers_top$gene) %in% rownames(sc[["RNA"]])])
        valid_proteins = gsub("ADT_", "", markers_top$gene[gsub("ADT_", "", markers_top$gene) %in% rownames(sc[["ADT"]])])
        
        # Produce plots
        if(length(valid_genes)>0){
          p_rna = Seurat::DotPlot(sc, features=valid_genes %>% unique(), 
                                  assay="RNA", 
                                  scale = ifelse(type=="DotPlotScaled", TRUE, FALSE)) + 
            ...
            guides(size=guide_legend(order=1, title="Pct expressed"),
                   color = guide_colorbar(
                     title =ifelse(type=="DotPlotScaled", "Scaled Expression", 
                                                   "Average Expression")))
          caption = paste0("Top RNA markers per cluster (expression", 
                           ifelse(type=="DotPlotScaled", "scaled)", " unscaled)"))
        } else {
          p_rna_scaled = ggplot() + theme_void()
          caption = paste0("Top RNA markers per cluster (expression", ifelse(type=="DotPlotScaled", "scaled)", " unscaled)"))
        }
        
        if(length(valid_proteins)>0){
          p_adt = Seurat::DotPlot(sc, features=valid_proteins %>% unique(), 
                                  assay="ADT",
                                  scale = ifelse(type=="DotPlotScaled", TRUE, FALSE)) + 
            viridis::scale_color_viridis() + 
            AddPlotStyle(ylab="Cluster", xlab="", legend_position="bottom") + 
          ...
          caption = paste0("Top ADT markers per cluster (expression ", ifelse(type=="DotPlotScaled", "scaled)", "unscaled)"))
        } else {
          p_adt = ggplot() + theme_void()
          caption = paste0("Top ADT markers per cluster (expression ", ifelse(type=="DotPlotScaled", "scaled)", "unscaled)"))
        }
...
if (any(species_tibble$species_name == species)) {
  ...
  markers_up_per_cluster = split(gsub("RNA_", "", markers_filt$up$gene), 
                                 markers_filt$up$cluster)
  markers_down_per_cluster = split(gsub("RNA_", "", markers_filt$down$gene), 
...
A combined feature plot visualises the combined score of each gene list on a single-cell level. The combined score is calculated based on the Mann-Whitney U statistic using UCell. Scores can range between 0 and 1. The closer a score gets to 1, the more genes of a list are among the top expressed genes.
