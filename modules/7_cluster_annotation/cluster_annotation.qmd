...
if (markers_found > 0) {
  ...
  for (name in names(known_markers_list)) {
      p1 = Seurat::FeaturePlot(sc, features=paste0("UCell_", MakeNamesValid(name)), order=TRUE) +
          ...
          theme(legend.key.width = unit(0.02, "npc"))
      p1 = Seurat::LabelClusters(p1, id="ident", box=TRUE, fill="white", colour="black", label.padding=0.05)
...
"
    ...
    cat("::: panel-tabset\n")
    # Loop through lists
    for (lst in names(known_markers_list)) {
        cat("#####", lst, "\n")
        
        # Produce plots
        plist = Seurat::FeaturePlot(sc, features=known_markers_list[[lst]], combine=FALSE, order=TRUE)
        ...
        names(plist) = known_markers_list[[lst]]
