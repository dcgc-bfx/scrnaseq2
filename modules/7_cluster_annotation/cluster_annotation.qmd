...
The gene sets present in C8 collection covers various cell types from Heart, GI Tract, Pancreas, Kidney, Liver, the Immune system, Retina, Olfactory tissue, and the Brain. So do not be surprised if you see a cell type annotated here that should not be in your data.
...
if(any(species_tibble$species_name == species)) {
  ...
  if (!is.null(ora_up_results_combined) && nrow(ora_up_results_combined) > 0) {
    ...
    # 
    # # Save the workbook
    # saveWorkbook(wb, file.path(module_dir, "results", 
    #                            "marker_annotation_C8_ora.xlsx"), 
    #              overwrite = TRUE)
  } else {
...
if (length(ora_up_results) > 0) {
  # Save ORA results
  outfile <- file.path(module_dir, "results", "marker_annotation_C8_ora.xlsx")
  ...
  # Save the workbook
  invisible(saveWorkbook(wb, outfile, overwrite = TRUE))
