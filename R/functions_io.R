...
#' Read Metadata from a Table File
#'
#' Read metadata from delimited text, Excel, or RDS files and return it as a data frame keyed by the first column. For Excel inputs, an optional sheet number can be appended to the file path as `:<sheet_number>`.
#'
#' @param file Path to a character-separated file (`csv`, `tsv`, `txt`, optionally gzipped), an Excel file (`xls`, `xlsx`), or an RDS file containing a table. The first column must contain the barcode or feature identifier. Default: no default.
#' @return A data.frame containing the metadata, with row names set from the first column.
...
#'
#' Read a datasets table from a delimited text file or Excel workbook and perform dataset-specific validation checks before returning the imported table.
#'
#' @param file Path to a character-separated file (`csv`, `tsv`, optionally gzipped) or an Excel file (`xls`, `xlsx`). For Excel files, a sheet can be specified by appending `:<sheet_number>`. Default: no default.
#' @return A table containing the imported dataset metadata.
#' @note AI-assisted documentation
...
#' Read Counts Data Across Supported Technologies
#'
#' Read count matrices generated by supported SmartSeq, 10x, Parse Biosciences, and Scale Bio workflows, then attach assay, technology, and metadata attributes to each returned matrix.
#'
#' @param path Path to the counts input. Supported inputs include delimited text files, Matrix Market directories, HDF5 files, and H5AD files depending on `technology`. For 10x VisiumHD, this can also be the `binned_outputs` directory so that multiple bin sizes are imported. Default: no default.
#' @param technology Technology identifier. Supported values are `smartseq2`, `smartseq3`, `10x`, `10x_visium`, `10x_visiumhd`, `10x_xenium`, `parse`, and `scale`. Default: no default.
#' @param assays Assay name or names to read. When the source contains a single assay, this value is used to label the returned assay. Default: no default.
#' @param barcode_metadata Optional table of additional barcode metadata, or a list of such tables, where the first column contains barcode identifiers. Missing barcodes are filled with `NA`. Default: `NULL`.
#' @param feature_metadata Optional table of additional feature metadata, or a list of such tables, where the first column contains feature identifiers. Missing features are filled with `NA`. Default: `NULL`.
#' @param barcode_suffix Optional suffix appended to output barcodes after import. Default: `NULL`.
#' @param visiumhd_bin_sizes Bin sizes to read when `technology="10x_visiumhd"` and `path` points to `binned_outputs`. Default: `c(8, 16)`.
#' @return A named list containing one counts matrix per assay or bin size. Each element is typically a sparse matrix and may also carry barcode, feature, assay, technology, and path attributes.
#' @note AI-assisted documentation
...
#' Create a Seurat Segmentation from Coordinates
#'
#' Build a Seurat segmentation object from a coordinate table containing polygon vertices for each cell.
#'
#' @param coords Coordinate table with columns `cell`, `x`, and `y`, where each row represents one polygon vertex for a cell. Default: no default.
#' @return A SeuratObject `Segmentation` object created from the supplied cell polygons.
#' @note AI-assisted documentation
...
#'
#' Read spatial image-related data for supported 10x platforms and return the corresponding Seurat image object with the requested default assay applied.
#'
#' @param image_dir Path to the spatial or image directory for the selected technology. Default: no default.
#' @param technology Technology identifier. Supported values are `10x_visium`, `10x_visiumhd`, and `10x_xenium`. Default: no default.
#' @param assay Assay name to set as the default assay on the returned image object. Default: no default.
#' @param barcodes Named vector of barcodes to keep, reorder, and rename, where names are original barcodes and values are replacement barcodes. Pass `NULL` to keep all barcodes. Default: no default.
#' @param coordinate_type For `10x_xenium`, which coordinate representations to load. Default: `c("centroids", "segmentations")`.
#' @return A Seurat spatial image object, such as a Visium image or Xenium FOV, with its default assay set to `assay`.
#' @note AI-assisted documentation
...
#' Read Summary Metrics for a Dataset
#'
#' Read a metrics file produced by a supported sequencing or spatial transcriptomics workflow and dispatch it to the technology-specific parser.
#'
#' @param metrics_file Path to the metrics file to read. Default: no default.
#' @param technology Technology identifier. Supported values are `smartseq2`, `smartseq3`, `10x`, `10x_visium`, `10x_xenium`, `parse`, and `scale`. Default: no default.
#' @return One or more tables containing summary metrics parsed for the selected technology.
#' @note AI-assisted documentation
...
#' Parse Plate Layout Information from Cell Names
#'
#' Extract plate number, row, column, and remaining sample name information from cell identifiers, primarily for SmartSeq-style naming conventions.
#'
#' @param cell_names Character vector of cell names to parse. Default: no default.
#' @param pattern Regular expression with capture groups for plate number, row, and column. If the pattern does not match, the parsed fields are set to `NA`. Default: `"_(\\d+)_([A-Z])(\\d+)$"`.
#' @return A data.frame with columns `PlateNumber`, `PlateRow`, `PlateCol`, and `Rest`.
#' @note AI-assisted documentation
...
#' Save a Seurat Object with On-Disk Layers
#'
#' Save a Seurat object to an output directory and, when requested, copy or reuse associated on-disk matrix layers while updating the metadata needed to reload them later.
#'
#' @param sc Seurat object to save. Default: no default.
#' @param outdir Output directory for `sc.rds` and any associated on-disk layer directories. The directory is created if needed and must be empty before saving. Default: no default.
#' @param write_disk_data Whether to copy existing on-disk layers into `outdir`. Default: `TRUE`.
#' @param relative_paths Whether to store paths to on-disk layers relative to the saved `sc.rds` file. Default: `FALSE`.
#' @param compress Whether to compress the saved `sc.rds` file. Default: `FALSE`.
#' @return No return value. The function writes `sc.rds` and, optionally, associated on-disk layer directories to `outdir`.
#' @note AI-assisted documentation
...
#' Update On-Disk Matrix Directories in a Seurat Object
#'
#' Copy on-disk assay layers to a new directory, reload the moved matrices, and optionally update the `SaveSeuratRds` tool metadata stored in the Seurat object.
#'
#' @param sc Seurat object containing on-disk assay layers. Default: no default.
#' @param dir Target directory for copied on-disk layer data. Default: no default.
#' @param assays Assay names whose on-disk layers should be copied. If `NULL`, all assays are processed. Default: `NULL`.
#' @param layer Layer name or pattern identifying which layers to copy. If `NULL`, all layers in the selected assays are processed. Default: `NULL`.
#' @param update_tool_saveseuratrds Whether to update the `SaveSeuratRds` tool entry with the new layer paths. Default: `FALSE`.
#' @return The Seurat object with updated on-disk layer references.
#' @note AI-assisted documentation
...
#' Export a Seurat Object to Loupe Format
#'
#' Create a Loupe Browser file from a Seurat object by exporting counts, categorical metadata, and selected dimensionality reductions.
#'
#' @param sc Seurat object to export. Default: no default.
#' @param assay Assay to include in the Loupe file. If `NULL`, the default assay is used. Default: `NULL`.
#' @param categories Cell metadata columns to include. If `NULL`, all non-numeric metadata columns are used; numeric columns are always excluded. Default: `NULL`.
#' @param embeddings Embedding reductions to include. If `NULL`, all reductions in the Seurat object are exported. Default: `NULL`.
#' @param barcodes Barcodes to include in the export. If `NULL`, all barcodes from the selected assay are used. Default: `NULL`.
#' @param output_dir Directory where the Loupe file will be written. Default: `"."`.
#' @param output_name Output file name for the Loupe export. Default: `"cloupe.cloupe"`.
#' @return The result returned by `loupeR::create_loupe`, typically indicating whether the Loupe file was created successfully.
#' @note AI-assisted documentation
...
#' Export Seurat Metadata to Xenium Explorer Format
#'
#' Export categorical cell metadata from a Seurat object into Xenium Explorer `analysis.zarr.zip` files so that grouped annotations can be reused in the native explorer format.
#'
#' @param sc Seurat object to export. Default: no default.
#' @param assay Assay whose cells should be exported. If `NULL`, the default assay is used. Default: `NULL`.
#' @param categories Cell metadata columns to include. If `NULL`, all non-numeric metadata columns are used; numeric columns are always excluded. Default: `NULL`.
#' @param barcodes Barcodes to include in the export. If `NULL`, all barcodes from the selected assay are used. Default: `NULL`.
#' @param output_dir Directory where Xenium Explorer output files will be written. Default: `"."`.
#' @param output_name Output file name to use for the Xenium Explorer archive. Default: `"analysis.zar.zip"`.
#' @return No return value. The function writes Xenium Explorer analysis archives beneath `output_dir`.
#' @note AI-assisted documentation
